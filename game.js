...
defaults.divineRoamingUnlocked=false;defaults.divineRoamingManualCount=0;defaults.divineRoamingDay='';defaults.divineRoamingUsed=0;defaults.divineRoamingJob=null;defaults.divineRoamingHarvest={};
...
function bodyTrainingOptions(){const realm=Math.floor((state.bodyLevel||0)/10)+1,need=bodyTemperNeed(),injuryReduction=bodyPassiveUnlocked(3)?10:0;return {
  basic:{name:'基礎鍛體',stamina:10,food:40*realm,wood:0,stone:0,gain:bodyTemperGain(need*.018),risk:0,description:'最節省額外材料的安全鍛體，不會造成傷勢。'},
  bath:{name:'藥浴鍛體',stamina:20,food:70*realm,wood:40*realm,stone:0,gain:bodyTemperGain(need*.04),risk:0,description:'以食物與木材調製藥浴，減少操作次數並治癒擦傷。'},
  extreme:{name:'極限鍛體',stamina:30,food:120*realm,wood:0,stone:0,gain:bodyTemperGain(need*.065),risk:Math.max(10,30-injuryReduction),description:'每點體力收益最高，但可能留下傷勢。'}
}}
...
function healBodyInjury(){const id=activeBodyInjury();if(!id)return toast('目前沒有傷勢');const costs=bodyHealCost(id);if(state.food<costs.food||state.wood<costs.wood)return toast('療傷所需食物或木材不足');state.food-=costs.food;state.wood-=costs.wood;state.bodyInjury='';state.bodyInjuryUntil=0;toast('傷勢已痊癒');renderExperiencePanel('body');render();save()}
...
function save() { if(suppressSave)return;const now=gameNow();state.lastSave=now;if(trustedClockReady)state.lastTrustedTime=Math.max(state.lastTrustedTime||0,now);localStorage.setItem(saveKey,JSON.stringify(state,(_,value)=>typeof value==='bigint'?value.toString():value)); }
...
function beginFirstBreakthrough(){
  ...
  setTimeout(()=>{state.free-=600n;state.cultivationAwakened=true;breakthroughInProgress=false;$('#heroArt').classList.remove('breakthrough-absorb');render();save();toast('已突破至聽息・一層，新手指引完成');},2100);
}
...
async function startGame() {
  ...
  if(away>0&&state.cultivationAwakened) { const gain=offlineCultivationGain(savedLast,now),swordGain=offlineSwordEssenceGain(away);addAura(away*auraRate());state.swordEssence+=toBigInt(swordGain);runSettlementTick(away);addCultivation(gain,true);setTimeout(()=>{if(sessionOnline)showOfflineRewards(offlineBefore,away*5)},180); }
...
function combatTechniqueAvailable(move){return !!move&&(move.id==='origin'||swordTechniqueUnlocked(move.id))}
...
async function chooseSwordEmbryo(id){
  ...
  state.swordEmbryo=id;state.swordName=`${state.name||''}之劍`.slice(0,12)||'無名靈劍';state.swordMoves=['origin',swordTechniquesForEmbryo(id)[0]?.id].filter(Boolean);normalizeSwordPath();toast(`已凝聚${embryo.name}`);renderExperiencePanel('sword');render();save();
}
...
function processDivineRoaming(){divineRoamingDaily();const job=state.divineRoamingJob;if(!job||job.completed>=job.total)return false;const now=gameNow(),due=Math.min(job.total-job.completed,Math.max(0,Math.floor((now-job.nextAt)/divineRoamingAttemptMs)+1));if(!due)return false;const stage=mortalMainline[job.stageId-1];for(let i=0;i<due;i++){const attempt=(state.divineRoamingUsed||0)+1;rollDivineRoamingDrop(stage,divineRoamingRate(attempt));state.divineRoamingUsed=attempt;job.completed++;job.nextAt+=divineRoamingAttemptMs}if(job.completed>=job.total)job.finishedAt=now;save();return true}
function divineRoamingStatus(){processDivineRoaming();const job=state.divineRoamingJob;if(!job)return '神念歸體・目前閒置';const stage=mortalMainline[job.stageId-1];return job.completed>=job.total?`${stage.name}遠遊完成・${job.completed}/${job.total}`:`正遠遊${stage.name}・${job.completed}/${job.total}`}
...
function openDivineRoamingStage(id){if(!state.divineRoamingUnlocked)return openDivineRoamingUnlock();processDivineRoaming();const stage=mortalMainline[id-1],daily=divineRoamingDaily(),job=state.divineRoamingJob,active=job&&job.completed<job.total;let modal=$('#divineRoamingModal');if(!modal){modal=document.createElement('div');modal.id='divineRoamingModal';modal.className='divine-roaming-modal';document.body.append(modal)}modal.innerHTML=`<section class="divine-roaming-window"><button class="divine-close">×</button><h2>${stage.name}・神念遠遊</h2><p>${divineRoamingStatus()}</p><div class="divine-status-grid"><span>今日已遠遊<b>${daily.used} / 100</b></span><span>剩餘次數<b>${daily.remaining}</b></span><span>當前疲勞<b>${daily.used<50?'清明・100%':daily.used<75?'疲憊・50%':'衰弱・10%'}</b></span></div><div class="divine-counts">${[1,10,100].map(n=>`<button data-divine-count="${n}" ${active||daily.remaining<n?'disabled':''}>遠遊 ${n} 次</button>`).join('')}</div><small>每次遠遊需現實 1 分鐘；收益先存入遠遊臨時儲物袋，可隨時提取。</small></section>`;modal.classList.add('show');modal.querySelector('.divine-close').onclick=()=>modal.classList.remove('show');modal.querySelectorAll('[data-divine-count]').forEach(button=>button.onclick=()=>startDivineRoaming(stage,+button.dataset.divineCount))}
function startDivineRoaming(stage,total){const daily=divineRoamingDaily(),job=state.divineRoamingJob;if(job&&job.completed<job.total)return toast('神念仍在遠遊中');if(total>daily.remaining)return toast('今日神念遠遊次數不足');state.divineRoamingJob={stageId:stage.id,total,completed:0,startedAt:gameNow(),nextAt:gameNow()+divineRoamingAttemptMs};save();openDivineRoamingStage(stage.id);toast(`神念已前往${stage.name}`)}
...
function startMainlineBattle(stage){
  clearTimeout(battleTimer);startBgm('battle');const player=battlePlayerStats(),scale=.72+stage.id*.075,hpMod=stage.id%4===0?1.16:1,attackMod=stage.id%4===3?1.12:1,defenseMod=stage.id%4===2?1.12:1;state.mainlineEnemySnapshots=state.mainlineEnemySnapshots||{};let enemy=state.mainlineEnemySnapshots[stage.id];if(!enemy){enemy={maxHp:Math.max(40,Math.round(player.maxHp*scale*hpMod)),attack:Math.max(8,Math.round(player.attack*scale*attackMod)),defense:Math.max(2,Math.round(player.defense*(.7+stage.id*.045)*defenseMod)),accuracy:Math.max(1,Math.round(player.accuracy*(.72+stage.id*.025))),evasion:Math.max(0,Math.round(player.evasion*(.7+stage.id*.02))),crit:Math.min(.22,.04+stage.id*.008),combatPower:Math.round(combatPower()*scale)};state.mainlineEnemySnapshots[stage.id]={...enemy};save()}else enemy={...enemy};
  const firstName=mainlineWaves[stage.id-1][0],firstEnemy={...enemy,maxHp:Math.max(20,Math.round(enemy.maxHp*.58)),attack:Math.max(6,Math.round(enemy.attack*.62)),defense:Math.round(enemy.defense*.68)};battle={active:true,resolved:false,mode:'mainline',mainlineStage:stage,bossTemplate:{...enemy},waveIndex:0,mechanicTriggered:false,round:1,completedRounds:0,playerMoveIndex:0,player:{...player,hp:player.maxHp},enemy:{...firstEnemy,hp:firstEnemy.maxHp,name:firstName,npc:{id:`mainline-${stage.id}-0`},race:stage.race},logs:[]};$('#battleModal').classList.remove('hidden');$('#battleStage').classList.remove('hidden');$('#battleResult').classList.add('hidden');$('#playerSilhouette').className=`battle-silhouette ${state.gender==='女'?'silhouette-player-female':'silhouette-player-male'}`;$('#enemySilhouette').className=`battle-silhouette silhouette-${stage.race}`;$('#battlePlayerName').textContent=state.name;$('#battleEnemyName').textContent=firstName;$('.battle-arena').style.backgroundImage=`linear-gradient(#edf5ef88,#e4ddca55),url('${stage.image}')`;$('#battleLog').innerHTML=`<p><b>${stage.name}</b>・三段遭遇</p><p><b>關卡機制：</b>${mainlineMechanics[stage.id-1]}</p>`;syncBattleWeapon();updateBattleUi();battleTimer=setTimeout(playerBattleTurn,700);
}
function advanceMainlineWave(){const stage=battle.mainlineStage,next=++battle.waveIndex,names=[...mainlineWaves[stage.id-1],stage.boss],scale=next===1?.8:1,template=battle.bossTemplate,enemy={...template,maxHp:Math.max(30,Math.round(template.maxHp*scale)),attack:Math.max(7,Math.round(template.attack*(next===1?.82:1))),defense:Math.round(template.defense*(next===1?.86:1))};battle.enemy={...enemy,hp:enemy.maxHp,name:names[next],npc:{id:`mainline-${stage.id}-${next}`},race:stage.race};battle.mechanicTriggered=false;startBgm('battle');$('#battleEnemyName').textContent=names[next];appendBattleLog(next===2?`前路震動，Boss「${stage.boss}」現身，關卡進入最終階段。`:`精英「${names[next]}」接替上陣。`,'enemy');updateBattleUi();battleTimer=setTimeout(playerBattleTurn,900)}
...
function renderSwordMoves(inner){
  if(!state.swordEmbryo){
    const move=startingTechniques[0];
    inner.innerHTML=`<section class="move-loadout origin-loadout"><h2>出戰招式</h2><div class="equipped-moves"><span>第一式・<b>${move.name}</b></span></div><p>所有已掌握的招式都會永久保留。未來學會更多招式後，可在此自由搭配出戰順序。</p></section><div class="art-list"><article class="art-card move-card origin-move-card"><span class="art-element">氣</span><div class="move-copy"><b>${move.name}</b><p>${move.description}</p><em>傷害 ${Math.round(move.min*100)}%～${Math.round(move.max*100)}%</em></div><strong class="move-status">已掌握・第一式</strong></article></div>`;
    return
  }
  const equipped=(state.swordMoves||[]).map(combatTechniqueById),embryo=swordEmbryos[state.swordEmbryo],origin=startingTechniques[0],moveButton=(move,slot)=>`<button data-equip-move="${move.id}" data-slot="${slot}" ${state.swordMoves[slot]===move.id?'disabled':''}>設為第${slot+1}式</button>`;inner.innerHTML=`<section class="move-loadout"><h2>出戰招式配置</h2><div class="equipped-moves"><span>第一式・<b>${equipped[0]?.name||'未裝配'}</b></span><span>第二式・<b>${equipped[1]?.name||'未裝配'}</b></span></div><p>所有已掌握招式皆可自由搭配，戰鬥時依第一式、第二式循環施展。本命劍只會新增劍招，不會取代原有招式。</p></section><div class="sword-move-grid"><article class="origin-move-card"><header><b>${origin.name}</b><em>入道招式</em></header><p>${origin.description}</p><strong>氣芒直擊・傷害 ${Math.round(origin.min*100)}%～${Math.round(origin.max*100)}%</strong><small>已掌握・可自由裝配</small><div>${moveButton(origin,0)}${moveButton(origin,1)}</div></article>${swordTechniqueCatalog.map(move=>{const unlock=swordTechniqueUnlockStage(move.id),wrongEmbryo=move.embryo!==state.swordEmbryo,locked=wrongEmbryo||!swordTechniqueUnlocked(move.id),status=wrongEmbryo?`${swordEmbryos[move.embryo].short}專屬・目前尚未掌握`:unlock&&!swordTechniqueUnlocked(move.id)?`試劍境第 ${unlock} 關解鎖`:`傷害 ${Math.round(move.min*100)}%～${Math.round(move.max*100)}%`;return `<article class="${locked?'locked':''} embryo-${move.embryo}"><header><b>${move.name}</b><em>${swordEmbryos[move.embryo].short}・第${move.order}式</em></header><p>${move.description}</p><strong>${move.effect}</strong><small>${status}</small><div><button data-equip-move="${move.id}" data-slot="0" ${locked||state.swordMoves[0]===move.id?'disabled':''}>設為第一式</button><button data-equip-move="${move.id}" data-slot="1" ${locked||state.swordMoves[1]===move.id?'disabled':''}>設為第二式</button></div></article>`}).join('')}</div>`;$$('[data-equip-move]').forEach(button=>button.onclick=()=>setSwordMove(button.dataset.equipMove,+button.dataset.slot));
}
function renderArtsPanel(view='sect'){
  ...
  $('#featureDescription').innerHTML=`<div class="arts-main-tabs">${artMainTabs.map(([key,label])=>`<button data-art-view="${key}" class="${key===view||(key==='books'&&bookView)?'active':''}">${label}</button>`).join('')}</div>${subTabs}${wallet}<div id="artsInner"></div>`;
  $$('[data-art-view]').forEach(button=>button.onclick=()=>renderArtsPanel(button.dataset.artView));if(view==='sect'&&cap<40)$('#expandArtsBtn').onclick=expandArtsCapacity;
...
function processSectYears(){
  ...
  if(task){state.sectContribution+=task.gain*delta;state.spiritStone+=task.stone*delta;state.prestige+=task.prestige*delta}
}
...
function renderSectPanel(view='home'){
  ...
  $$('.sect-tabs button').forEach(b=>b.onclick=()=>renderSectPanel(b.dataset.sectView));renderSectView(view);renderSectPathIncome();save();
}
...
function battlePlayerStats(){
  ...
  const marks=swordPathMarkCounts(),swordPower=(1+(state.swordLevel||0)*.008)*(1+Math.min(.2,marks.evil*.02)),guardPower=1+Math.min(.2,marks.righteous*.02);
  return {
    maxHp:Math.round(combatHealth(rootBone)*(activeBodyInjury()==='internal'?.85:1)),attack:Math.max(12,trueQi*5)*swordPower,defense:Math.max(0,physique*20)*guardPower,
...
function enemyBattleTurn(){
  if(!battle?.active)return;if(battle.mode==='mainline'){const stage=battle.mainlineStage;if(battle.completedRounds>0&&battle.completedRounds%3===0){battle.enemy.attack=Math.round(battle.enemy.attack*1.06);appendBattleLog(`${stage.boss}引動「${mainlineMechanics[stage.id-1].split('：')[0]}」，攻勢再度提升。`,'enemy')}if(!battle.mechanicTriggered&&[7,17,18].includes(stage.id)&&battle.enemy.hp<=battle.enemy.maxHp*.5){battle.mechanicTriggered=true;battle.enemy.attack=Math.round(battle.enemy.attack*1.15);battle.enemy.defense=Math.round(battle.enemy.defense*1.1);appendBattleLog(`${stage.boss}氣息驟變，關卡核心機制進入第二階段。`,'enemy')}}const technique=startingTechniques[Math.floor(Math.random()*startingTechniques.length)],mult=technique.min+Math.random()*(technique.max-technique.min),hit=damageRoll(battle.enemy,battle.player,mult);
...
function caveCultivationBonus(){return state.caveCultivationEnabled&&state.cultivationAwakened?.08+state.caveCultivationLevel*.02:0}
...
function renderMarket(tab=currentMarketTab){
  ...
  const data={
    ...
    treasure:{title:'百寶樓',subtitle:'仙珍奇物',currency:'jade',products:['divineRoamingManual']}
  }[tab];
...
setInterval(()=>{if($('#gameScreen').classList.contains('active')){if(state.cultivationAwakened){addAura(auraRate());if(state.swordPathOpened)state.swordEssence+=BigInt(swordEssenceRate());runSettlementTick();processSectYears();addCultivation(rate())}processDivineRoaming();if(currentFeature==='root')renderSpiritRootView(currentRootView);if(currentFeature==='cave'&&state.cultivationAwakened)renderCavePanel(currentCaveView);if(currentFeature==='sect'&&currentSectView!=='npcs')renderSectPanel(currentSectView);if(currentFeature==='arts')updateArtsLive();if(currentFeature==='experience'&&currentExperienceView==='realm')renderExperiencePanel('realm');tickStart=gameNow()}},5000);
