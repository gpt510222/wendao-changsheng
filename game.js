...
function ensureAscensionModal(){let modal=$('#ascensionModal');if(modal)return modal;modal=document.createElement('div');modal.id='ascensionModal';modal.className='ascension-modal';modal.innerHTML='<section class="ascension-window"><button class="ascension-close" type="button" aria-label="離開天路">×</button><div id="ascensionContent"></div></section>';document.body.append(modal);modal.querySelector('.ascension-close').onclick=()=>{if(modal.dataset.exitLocked==='true')return;modal.classList.remove('show');resumeWorldBgm()};return modal}
function setAscensionExitLocked(locked){const modal=ensureAscensionModal(),close=modal.querySelector('.ascension-close');modal.dataset.exitLocked=locked?'true':'false';close.hidden=!!locked}
const ascensionExitObserver=new MutationObserver(()=>{const modal=$('#ascensionModal'),content=$('#ascensionContent');if(!modal?.classList.contains('show')||!content)return;const stage=content.querySelector('.ascension-dialogue-stage'),chapter=content.querySelector('.ascension-chapter-card small')?.textContent||'';if(stage){const prologue=chapter.includes('序章');setAscensionExitLocked(!prologue);if(chapter.includes('照妄未破')||chapter.includes('天路未盡'))modal.dataset.exitAfterStory='true';return}if(content.querySelector('.ascension-allocation')){setAscensionExitLocked(true);return}if(modal.dataset.exitAfterStory==='true'){delete modal.dataset.exitAfterStory;setAscensionExitLocked(false);modal.classList.remove('show');resumeWorldBgm();return}setAscensionExitLocked(false)});ascensionExitObserver.observe(document.body,{childList:true,subtree:true});
...
function ascensionLine(speaker,text,portrait='player',extra={}){return {speaker:speaker==='808'?(state.name||'修士'):speaker,text,portrait,...extra}}
function playAscensionStory({theme,chapter,title,background,className='',lines,onDone}){startBgm(theme);const c=$('#ascensionContent');let step=0;const draw=()=>{const line=lines[step];if(!line){onDone?.();return}const choices=line.choices||[],body=choices.length?`<div class="ascension-story-choices">${choices.map((choice,index)=>`<button data-story-choice="${index}">${choice.label}</button>`).join('')}</div>`:`<button class="ascension-story-next" data-story-next>${step===lines.length-1?'繼續':'下一句'}</button>`;const portrait=(ascensionArt[line.portrait]||ascensionArt.player)();c.innerHTML=ascensionDialogueStage({chapter,title,background,portrait,speaker:line.speaker,text:line.text,body,className});if(choices.length)c.querySelectorAll('[data-story-choice]').forEach(button=>button.onclick=()=>{const choice=choices[+button.dataset.storyChoice];choice.onSelect?.();if(choice.followup?.length)lines.splice(step+1,0,...choice.followup);step++;draw()});else c.querySelector('[data-story-next]').onclick=()=>{step++;draw()}};draw()}
...
const heartRouteRemarks={qi:['你以天地之息養一身長生，最怕的往往不是氣盡，而是不知為何還要繼續吐納。','練氣者借天地成己身。借得越多，越要記得，有些東西從來不屬於你。'],sword:['劍修可以日日磨劍，卻未必日日都知道自己為何拔劍。','劍最擅長的是捨。可真正難的，是知道什麼能斬，什麼不能斬。'],body:['肉身可以承受千錘百煉，可若不知道為何受苦，再強的筋骨也只是更耐打的軀殼。','煉體者最習慣承受代價。可不是所有傷都該由你硬扛，也不是所有苦都值得受。']};
