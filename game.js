...
function renderAscensionSuccess(){const c=$('#ascensionContent'),titleId=syncAscensionTitleUnlock(),title=titleCatalog.find(item=>item.id===titleId);c.innerHTML=`<div class="ascension-success"><small>飛升成功</small><h2>凡途已盡，仙路初開</h2><p>「你以為天路盡頭，是因為道走完了？」</p><p>「這裡只是凡間的路走完了。門後的路，才剛開始。」</p><hr>${title?`<strong>獲得稱號「${title.name}」</strong>`:''}<strong>仙門之後沒有仙霧，也沒有靈潮。</strong><p>迎接你的，是一片靈氣枯竭、萬物死寂的荒蕪仙界。</p><em>凡間的修練、洞府生產與所有既有產能均不受影響。</em><button data-enter-immortal>踏入荒蕪仙界</button></div>`;c.querySelector('[data-enter-immortal]').onclick=()=>{normalizeAscension().currentRealm='immortal';ensureAscensionModal().classList.remove('show');save();render();startBgm('immortalBarren');toast('仙界沒有靈氣。這裡的路，要用另一種方式走。')}}
