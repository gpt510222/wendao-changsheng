...
function immortalHeader(eyebrow,title,description){const im=immortalState();return `<header class="immortal-page-head"><small>${eyebrow}</small><h2>${title}</h2><p>${description}</p><div class="immortal-resource-row"><span>殘界源質 <b>${im.residualSource}</b></span><span>荒痕 <b>${im.barrenTraces}</b></span><span>探索 <b>${immortalExploration()}%</b></span></div></header>`}
...
function renderImmortalRestoration(){const im=immortalState(),stage=im.guideStoneStage;$('#featureDescription').innerHTML=`<section class="immortal-play-page">${immortalHeader('殘界待甦・第一修復','引道碑','修復不是把仙界變回從前，而是讓已經斷裂的世界重新有路可走。')}<div class="guide-stone-visual stage-${stage}"><i></i><b>${stage>=3?'引道微明':stage?'殘紋復燃':'死寂碑身'}</b><span>${stage} / 3</span></div><div class="restoration-stages">${immortalRestorationStages.map((item,index)=>`<article class="${stage>index?'complete':stage===index?'current':''}"><i>${stage>index?'✓':index+1}</i><div><b>${item.name}</b><span>${stage>index?'已完成':item.cost}</span>${stage===index?`<div class="immortal-material-costs">${immortalRestorationCostHtml(index)}</div>`:''}</div>${stage===index?`<button data-restore-stage="${index}">修復</button>`:''}</article>`).join('')}</div>${stage>=3?'<div class="immortal-complete-note"><b>界應 +10</b><span>引道碑已將你的存在寫入這片天地。無息荒原的方位若隱若現。</span></div>':'<p class="immortal-hint">所需的仙界材料可由仙域調查與飛升古臺外圍遠征取得；素材會直接存入儲物袋。</p>'}</section>`;$('#featureDescription').querySelector('[data-restore-stage]')?.addEventListener('click',restoreGuideStone)}
function restoreGuideStone(){const im=immortalState(),stage=im.guideStoneStage;if(stage===0&&im.residualSource<2)return toast('殘界源質不足');if(stage===1&&(im.barrenTraces<1||state.meteorIron<300))return toast('需要荒痕 ×1、隕鐵 ×300');if(stage===2&&(im.residualSource<4||im.barrenTraces<2||state.wood<500))return toast('需要殘界源質 ×4、荒痕 ×2、木材 ×500');if(stage===0)im.residualSource-=2;else if(stage===1){im.barrenTraces--;state.meteorIron-=300}else{im.residualSource-=4;im.barrenTraces-=2;state.wood-=500}im.guideStoneStage++;const scripts=[['碑基之下','你剝去碑基上的死灰，發現石中埋著與天路同源的古紋。','這座碑不是路標。它曾把新來者的存在，寫入仙界法則。'],['斷紋重接','凡間隕鐵一觸碑身便迅速黯淡，卻替殘紋搭起短暫的橋。','凡物在仙界會衰敗，但並非毫無作用。你的舊路，仍能成為新路的材料。'],['引道微明','最後一道殘紋亮起。無靈荒野沒有風，遠方沙塵卻第一次向兩側分開。','碑接受了你的存在。界應初生，而路的盡頭指向一片名為「無息荒原」的死地。']][stage];playImmortalStory({chapter:`復界・第 ${stage+1} 階`,title:scripts[0],lines:[ascensionLine('旁白',scripts[1],'none'),ascensionLine('天路之主',scripts[2],'master'),...(stage===2?[ascensionLine('系統','取得界應 10。仙界戰力壓制略微緩和。','none')]:[])],onDone:()=>{toast(stage===2?'引道碑修復完成・界應 +10':'引道碑修復進度提升');renderImmortalRestoration()}})}
