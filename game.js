...
window.WENDAO_BUILD='20260915-136';
...
async function syncServerJade(){const channel=leaderboardConfig.sessionKey.includes('release')?'formal':'test',wallet=await playerStateRpc('player_jade_bootstrap',{p_channel:channel,p_legacy_balance:Math.max(0,Number(state.spiritJade)||0)});state.spiritJade=Math.max(0,Number(wallet?.balance)||0);serverJadeRevision=Number(wallet?.revision)||serverJadeRevision;return wallet}
...
async function startGame() {
  if(sessionOnline)return;finishPause();let serverSettlement;try{serverSettlement=await connectServerPlayerState();await playerIdentityBootstrap();await playerAscensionBootstrap();await playerImmortalBootstrap();await playerCoreAttributesGet();await playerMoralBootstrap();await playerEncounterSync()}catch(error){toast(`需要連線伺服器才能進入遊戲：${error.message}`);return}sessionOnline=true;await importLegacyQiRuntime();await playerQiRuntime();await playerSectBootstrap();await playerSectProgressBootstrap();await playerSectEconomy();await playerSectShopStatus();await syncServerJade();
