...
function finishBattle(won,reason){
  ...
  if(battle.mode==='ascension-delusion'||battle.mode==='ascension-road-end')setTimeout(()=>completeAscensionBattle(battle.mode,won),180);
...
function normalizeAscension(){const base=defaults.ascension,current=state.ascension&&typeof state.ascension==='object'?state.ascension:{};state.ascension={...base,...current,heartAnswers:Array.isArray(current.heartAnswers)?current.heartAnswers.slice(0,3):[],delusionReplies:Array.isArray(current.delusionReplies)?current.delusionReplies.slice(0,3):[]};return state.ascension}
...
function ensureAscensionModal(){let modal=$('#ascensionModal');if(modal)return modal;modal=document.createElement('div');modal.id='ascensionModal';modal.className='ascension-modal';modal.innerHTML='<section class="ascension-window"><button class="ascension-close" type="button" aria-label="離開天路">×</button><div id="ascensionContent"></div></section>';document.body.append(modal);modal.querySelector('.ascension-close').onclick=()=>{modal.classList.remove('show');resumeWorldBgm()};return modal}
function ascensionDialogueStage({chapter,title,background,portrait,speaker,text,body='',className=''}){return `<section class="ascension-dialogue-stage ${className}" style="--ascension-bg:url('${background}')"><div class="ascension-chapter-card"><small>${chapter}</small><b>${title}</b></div><img class="ascension-dialogue-portrait" src="${portrait}" alt="${speaker} Q版半身立繪"><div class="ascension-dialogue-box"><strong>${speaker}</strong><p>${text}</p>${body}</div></section>`}
function renderAscensionEntrances(){const a=normalizeAscension(),eligible=ascensionEligible();$('#ascensionButton').classList.toggle('hidden',!eligible||a.ascended);$('#realmSwitchButton').classList.toggle('hidden',!a.ascended);const immortal=a.currentRealm==='immortal'&&a.ascended,$scene=$('.scene-bg');$('#gameScreen').classList.toggle('immortal-realm',immortal);if(immortal){$scene.src='assets/qstyle-v2/ascension/immortal-realm-barren-v1.png';$scene.alt='靈氣枯竭的荒蕪仙界';$('#realmSwitchButton span').textContent='凡間';$('#realmSwitchButton small').textContent='返回道場';$('#headerSpiritRealm').title='仙界沒有靈氣；凡間修練與產能照常運作'}else{const path=a.lastMortalTrainingGround||state.activePath||state.firstPath||'spirit';$scene.src=cultivationPathMeta[path]?.scene||cultivationPathMeta.spirit.scene;$scene.alt=`${cultivationPathMeta[path]?.name||'練氣'}修練道場`;$('#realmSwitchButton span').textContent='仙界';$('#realmSwitchButton small').textContent='登臨上界'}}
function switchWorldRealm(){const a=normalizeAscension();if(!a.ascended)return;a.currentRealm=a.currentRealm==='immortal'?'mortal':'immortal';if(a.currentRealm==='immortal')a.lastMortalTrainingGround=state.activePath||state.firstPath||a.lastMortalTrainingGround||'spirit';save();render();resumeWorldBgm();toast(a.currentRealm==='immortal'?'你踏入仙界。四野死寂，此地沒有一絲靈氣。':'你自荒蕪仙界降回凡間；凡間一切產能照常運轉。')}
...
function renderAscensionRoad(){const a=normalizeAscension(),content=$('#ascensionContent');if(a.pendingReward){renderAscensionAllocation(a.pendingReward);return}if(a.ascended){renderAscensionSuccess();return}const stages=[['序章｜天路將起',a.prologueCompleted,()=>openAscensionPrologue()],['問心陣',a.heartTrialCompleted,()=>openHeartTrial()],['照妄陣',a.delusionTrialCompleted,()=>openDelusionTrial()],['天路盡頭',a.roadEndCompleted,()=>openRoadEnd()]],unlocked=[true,a.prologueCompleted,a.heartTrialCompleted&&a.heartRewardAllocated,a.delusionTrialCompleted&&a.delusionRewardAllocated];content.innerHTML=`<header class="ascension-heading"><small>飛升・闖天路</small><h2>${a.route==='none'?'凡間的路，尚未走完':ascensionRouteMeta[a.route].name}</h2><p>九鎖已解，門仍未開。問心、照妄，而後以此身承擔自己的選擇。</p></header><div class="ascension-stage-grid">${stages.map((s,i)=>`<button data-asc-stage="${i}" ${!unlocked[i]||s[1]?'disabled':''}><i>${s[1]?'✓':i+1}</i><b>${s[0]}</b><small>${s[1]?'已通關・不可重入':unlocked[i]?'可進入':'尚未解鎖'}</small></button>`).join('')}</div>`;content.querySelectorAll('[data-asc-stage]').forEach(button=>button.onclick=stages[+button.dataset.ascStage][2])}
function openAscensionPrologue(){startBgm('ascensionPrologue');const c=$('#ascensionContent'),body=`<div class="ascension-route-grid"><button data-route="qi"><b>練氣</b><small>借天地之息，亦記得歸還天地</small></button><button data-route="sword"><b>淬劍</b><small>知何時出劍，也知何時不斬</small></button><button data-route="body"><b>煉體</b><small>承受痛苦，卻不把痛苦當作道</small></button></div><button class="ascension-secondary" data-back>待我思考</button>`;c.innerHTML=ascensionDialogueStage({chapter:'序章・天路將起',title:'九鎖崩解',background:'assets/qstyle-v2/ascension/bg-prologue-tianlu-v1.png',portrait:'assets/qstyle-v2/mainline/portrait-lord.png',speaker:'守界司之主',text:'九鎖解開，只證明你替後來的人爭回了選擇。真正的天路，現在才開始。選定你的道路，走到門前。',body});c.querySelector('[data-back]').onclick=renderAscensionRoad;c.querySelectorAll('[data-route]').forEach(button=>button.onclick=()=>{const a=normalizeAscension();a.route=button.dataset.route;a.prologueCompleted=true;save();renderAscensionRoad();toast(`已選定${ascensionRouteMeta[a.route].name}・此選擇不可更改`)})}
function openHeartTrial(){startBgm('ascensionHeart');const c=$('#ascensionContent'),a=normalizeAscension(),body=`<div class="ascension-question-scroll">${heartQuestions.map((q,i)=>`<fieldset><legend>${i+1}. ${q.question}</legend>${q.options.map((o,j)=>`<label><input type="radio" name="heart-${i}" value="${j}" ${a.heartAnswers[i]===j?'checked':''}><span>${o}</span></label>`).join('')}</fieldset>`).join('')}</div><button data-heart-submit>留下此刻的答案</button>`;c.innerHTML=ascensionDialogueStage({chapter:'飛升・第 1 關',title:'問心陣',background:'assets/qstyle-v2/ascension/bg-heart-trial-v1.png',portrait:'assets/qstyle-v2/ascension/portrait-heart-spirit-v2.png',speaker:'問心陣靈',text:'此地沒有對錯。我只問三次，你只需留下此刻真正的答案。',body,className:'heart'});c.querySelector('[data-heart-submit]').onclick=()=>{const answers=heartQuestions.map((_,i)=>{const input=c.querySelector(`input[name="heart-${i}"]:checked`);return input?+input.value:null});if(answers.some(x=>x===null))return toast('請回答全部三問');a.heartAnswers=answers;a.heartTrialCompleted=true;a.pendingReward={stage:'heart',points:ascensionRouteMeta[a.route].reward[0]};save();renderAscensionAllocation(a.pendingReward)}}
function renderAscensionAllocation(reward){const a=normalizeAscension(),c=$('#ascensionContent'),points=Math.max(0,reward.points||0);c.innerHTML=`<div class="ascension-allocation"><small>${reward.stage==='heart'?'問心陣':reward.stage==='delusion'?'照妄陣':'天路盡頭'}・通關</small><h2>${reward.stage==='heart'?'答案無分對錯。心有所問，道才有所行。':reward.stage==='delusion'?'妄念不滅，識之即可。':'此路已盡，此道未終。'}</h2><p>將 ${points.toLocaleString()} 點自由分配至四項屬性；剩餘必須為 0。</p><b id="ascensionRemaining">剩餘 ${points.toLocaleString()}</b><div class="ascension-attribute-grid">${ascensionAttributes.map(([key,label])=>`<label><span>${label}</span><button type="button" data-minus="${key}">－</button><input type="number" inputmode="numeric" min="0" max="${points}" value="0" data-allocate="${key}"><button type="button" data-plus="${key}">＋</button></label>`).join('')}</div><button data-allocation-complete disabled>${reward.stage==='roadEnd'?'踏入仙門':'完成分配'}</button></div>`;const inputs=[...c.querySelectorAll('[data-allocate]')],complete=c.querySelector('[data-allocation-complete]');function refresh(){let used=0;inputs.forEach(input=>{input.value=String(Math.max(0,Math.min(points,Math.floor(Number(input.value)||0))));used+=+input.value});if(used>points){const changed=document.activeElement?.matches?.('[data-allocate]')?document.activeElement:inputs.at(-1);changed.value=String(Math.max(0,+changed.value-(used-points)));used=points}$('#ascensionRemaining').textContent=`剩餘 ${(points-used).toLocaleString()}`;complete.disabled=used!==points}inputs.forEach(input=>input.oninput=refresh);c.querySelectorAll('[data-minus]').forEach(b=>b.onclick=()=>{const i=c.querySelector(`[data-allocate="${b.dataset.minus}"]`);i.value=Math.max(0,+i.value-1);refresh()});c.querySelectorAll('[data-plus]').forEach(b=>b.onclick=()=>{const i=c.querySelector(`[data-allocate="${b.dataset.plus}"]`);i.value=+i.value+1;refresh()});complete.onclick=()=>{if(complete.disabled)return;inputs.forEach(input=>state[input.dataset.allocate]=Math.max(0,(state[input.dataset.allocate]||0)+(+input.value||0)));if(reward.stage==='heart')a.heartRewardAllocated=true;else if(reward.stage==='delusion')a.delusionRewardAllocated=true;else{a.roadEndRewardAllocated=true;a.roadEndCompleted=true;a.ascended=true;a.currentRealm='immortal';a.serverAscensionRank=a.serverAscensionRank||1}delete a.pendingReward;save();render();reward.stage==='roadEnd'?renderAscensionSuccess():renderAscensionRoad()};refresh()}
function openDelusionTrial(){startBgm('ascensionDelusion');const a=normalizeAscension(),c=$('#ascensionContent'),portrait=state.gender==='女'?'portrait-heart-demon-female-v1.png':'portrait-heart-demon-male-v2.png',attacks=a.heartAnswers.map((answer,i)=>`<blockquote>${delusionAttacks[i][answer]}</blockquote>`).join(''),body=`<div class="ascension-question-scroll">${attacks}</div><div class="ascension-replies">${['也許你說得有一部分是真的。','你只是在替我的選擇找最惡意的解釋。','就算理由不純粹，那也是我的理由。'].map((text,i)=>`<button data-reply="${i}">${text}</button>`).join('')}</div>`;c.innerHTML=ascensionDialogueStage({chapter:'飛升・第 2 關',title:'照妄陣',background:'assets/qstyle-v2/ascension/bg-delusion-trial-v1.png',portrait:`assets/qstyle-v2/ascension/${portrait}`,speaker:'心魔',text:'不用嘴。用你選的這條道，把我打碎。',body,className:'delusion'});c.querySelectorAll('[data-reply]').forEach(button=>button.onclick=()=>{a.delusionReplies=[+button.dataset.reply];a.delusionAttemptCount++;save();startAscensionBattle('ascension-delusion')})}
function openRoadEnd(){startBgm('ascensionRoadEnd');const a=normalizeAscension(),c=$('#ascensionContent'),body='<button data-road-battle>出手・最終試煉</button>';c.innerHTML=ascensionDialogueStage({chapter:'飛升・最終關',title:'天路盡頭',background:'assets/qstyle-v2/ascension/bg-road-end-v1.png',portrait:'assets/qstyle-v2/ascension/portrait-tianlu-master-v2.png',speaker:'天路之主',text:'我不問你說自己是誰，只看你走到今日，有沒有資格把這條路繼續走下去。你打不贏我；我要看的，是你能逼我退到哪一步。',body,className:'road-end'});c.querySelector('[data-road-battle]').onclick=()=>{a.roadEndAttemptCount++;save();startAscensionBattle('ascension-road-end')}}
function startAscensionBattle(mode){ensureAscensionModal().classList.remove('show');clearTimeout(battleTimer);const player=battlePlayerStats(),roadEnd=mode==='ascension-road-end';startBgm(roadEnd?'ascensionRoadEnd':'ascensionDelusion');const ratio=roadEnd?1.08:.82,generated=npcCoreFromPower(Math.max(100,Math.round(combatPower()*ratio/5)*5),{id:mode,seedScope:mode}),core=generated.core,enemy={combatPower:generated.combatPower,core,maxHp:Math.round(combatHealth(core.rootBone)*(roadEnd?1.25:1)),attack:Math.max(12,core.trueQi*5),defense:Math.max(0,core.physique*20),evasion:combatEvasion(core.agility),accuracy:combatAccuracy(core.spiritualPower),crit:combatCritical(core.spiritualPower),name:roadEnd?'天路之主':'心魔',race:roadEnd?'immortal':state.gender==='男'?'human':'demon'};battle={active:true,resolved:false,mode,round:1,completedRounds:0,playerMoveIndex:0,player:{...player,hp:player.maxHp},enemy:{...enemy,hp:enemy.maxHp},logs:[]};$('#battleModal').classList.remove('hidden');$('#battleStage').classList.remove('hidden');$('#battleResult').classList.add('hidden');$('#playerSilhouette').className=`battle-silhouette ${state.gender==='女'?'silhouette-player-female':'silhouette-player-male'}`;$('#enemySilhouette').className=`battle-silhouette silhouette-${enemy.race}`;$('#battlePlayerName').textContent=state.name;$('#battleEnemyName').textContent=enemy.name;$('.battle-arena').style.backgroundImage=`linear-gradient(#10121455,#18140d55),url('assets/qstyle-v2/ascension/${roadEnd?'bg-road-end-v1.png':'bg-delusion-trial-v1.png'}')`;$('#battleLog').innerHTML=`<p><b>${roadEnd?'最終試煉':'照妄之戰'}</b>・${roadEnd?'將天路之主氣血壓至 10% 即獲認可':'擊敗映照自身的心魔'}</p>`;syncBattleWeapon();updateBattleUi();battleTimer=setTimeout(playerBattleTurn,700)}
function completeAscensionBattle(mode,won){const a=normalizeAscension();if(!won){save();return}if(mode==='ascension-delusion'){a.delusionTrialCompleted=true;a.pendingReward={stage:'delusion',points:ascensionRouteMeta[a.route].reward[1]}}else{a.pendingReward={stage:'roadEnd',points:ascensionRouteMeta[a.route].reward[2]}}save()}
function renderAscensionSuccess(){const c=$('#ascensionContent');c.innerHTML='<div class="ascension-success"><small>飛升成功</small><h2>凡途已盡，仙路初開</h2><p>「你以為天路盡頭，是因為道走完了？」</p><p>「這裡只是凡間的路走完了。門後的路，才剛開始。」</p><hr><strong>仙門之後沒有仙霧，也沒有靈潮。</strong><p>迎接你的，是一片靈氣枯竭、萬物死寂的荒蕪仙界。</p><em>凡間的修練、洞府生產與所有既有產能均不受影響。</em><button data-enter-immortal>踏入荒蕪仙界</button></div>';c.querySelector('[data-enter-immortal]').onclick=()=>{normalizeAscension().currentRealm='immortal';ensureAscensionModal().classList.remove('show');save();render();startBgm('immortalBarren');toast('仙界沒有靈氣。這裡的路，要用另一種方式走。')}}
