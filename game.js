...
function studyRecordsView(){
  ...
  return `<section class="study-section-heading"><div><small>不論成敗，皆為來路</small><h3>修行戰錄</h3></div><span>${cleared+swordWins+bodyTrials} 次關隘已過</span></section><div class="study-record-grid"><article><i>鎖</i><small>九鎖封天</small><b>${cleared}／18</b><p>${lastStage?`最遠抵達：${lastStage.name}`:'尚未踏入封天諸域'}</p></article><article><i>劍</i><small>試劍境</small><b>${swordWins}／${swordTrialMaxStage}</b><p>${swordRecord}</p></article><article><i>身</i><small>肉身試煉</small><b>${bodyTrials} 次破境</b><p>${state.bodyPathOpened?`現為${realmName(state.bodyLevel,bodyRealms)}・目前記錄敗陣 ${bodyFailures} 次`:'尚未踏上煉體之路'}</p></article><article><i>緣</i><small>因緣歲月</small><b>${encounters} 則</b><p>${encounters?'所作抉擇皆已收入典故錄':'尚未留下可供翻閱的因緣'}</p></article></div>${cleared?`<section class="study-milestones"><h4>破鎖紀要</h4>${mortalMainline.slice(0,cleared).reverse().map(stage=>`<article><span>第 ${stage.id} 關</span><b>${stage.name}</b><small>${mainlineAftermath[stage.id-1]}</small></article>`).join('')}</section>`:studyEmpty('破鎖紀要尚無墨跡','完成九鎖封天關卡後，結果會自動記在此處。')}`;
}
function studyChronicleView(){
  const cleared=Math.max(0,Math.min(mortalMainline.length,state.mainlineCleared||0)),histories=state.encounterHistory||[];
  ...
  const encounter=histories.length?`<section class="study-chronicle-group"><h4>因緣歲月</h4><div class="study-encounter-list">${histories.map(entry=>`<article><small>${entry.year?`修練第 ${entry.year} 年`:'行路有緣'}</small><b>${entry.title}</b><span>所擇・${entry.choice}</span><p>${entry.result}</p></article>`).join('')}</div></section>`:studyEmpty('因緣簿仍是空白','完成因緣抉擇後，經歷與結果會留存在此。');
  return `<section class="study-section-heading"><div><small>舊事可鑑，抉擇不可改</small><h3>典故錄</h3></div><span>主線 ${cleared}・因緣 ${histories.length}</span></section>${story}${encounter}`;
}
