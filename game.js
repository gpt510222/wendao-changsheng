...
function baseCore(attribute){return Math.max(0,(state[attribute]||0)+artDirectBonusFor(attribute)+swordPathBonus(attribute)+equippedAttributeBonus(attribute))}
...
function claimDivineHarvest(){const entries=Object.values(state.divineRoamingHarvest||{});if(!entries.length)return;const physicalCount=x=>x.type==='state'?x.key:craftingMaterialCountByName[x.key],newSlots=entries.filter(x=>{const count=physicalCount(x);return count&&Object.values(itemCatalog).some(item=>item.count===count)&&(state[count]||0)<=0}).length;if(bagUsedSlots()+newSlots>bagCapacity())return toast('角色儲物袋空間不足');entries.forEach(x=>{const count=physicalCount(x);if(count)state[count]=(state[count]||0)+x.amount;else{state.mainlineLoot=state.mainlineLoot||{};state.mainlineLoot[x.key]=(state.mainlineLoot[x.key]||0)+x.amount}});state.divineRoamingHarvest={};save();render();openDivineHarvest();toast('遠遊收益已提取至角色儲物袋')}
...
function grantMainlineDrops(stage){
  state.mainlineMaterials=state.mainlineMaterials||{};state.mainlineLoot=state.mainlineLoot||{};const odd=stage.id%2===1,results=[],addLoot=(name,amount=1)=>{const count=craftingMaterialCountByName[name];if(count)state[count]=(state[count]||0)+amount;else state.mainlineLoot[name]=(state.mainlineLoot[name]||0)+amount;results.push(`${name} ×${amount}`)},roll=(chance,fn)=>Math.random()<chance&&fn(),range=(min,max)=>min+Math.floor(Math.random()*(max-min+1));
...
function renderCavePanel(view='dwelling',preserveScroll=false){
  ...
  const tabs=[['dwelling','靈脈'],['production','資源生產'],['alchemy','丹房'],['forge','器室'],['brew','釀坊'],['study','書房'],['partner','道侶']];
...
function sortBagItems(items){
  if(state.bagSortMode!=='category')return items;
  return items.sort(([keyA,itemA],[keyB,itemB])=>{const a=bagItemSortProfile(keyA,itemA),b=bagItemSortProfile(keyB,itemB);return a.category-b.category||b.tier-a.tier||b.quality-a.quality||a.slot-b.slot||itemA.name.localeCompare(itemB.name,'zh-Hant')});
}
function organizeBag(){state.bagSortMode='category';save();renderBagView('bag');toast('儲物袋整理完成・之後取得的道具也會自動歸類')}
function renderBagView(view) {
  ...
  if(view==='bag') {
    const items=sortBagItems(Object.entries(itemCatalog).filter(([,item])=>(state[item.count]||0)>0)),capacity=bagCapacity(),used=items.length,cost=bagUpgradeCost();
    const itemButtons=items.map(([key,item])=>{const category=bagItemSortProfile(key,item).category;return `<button class="inventory-item" data-bag-item="${key}" data-bag-category="${category}" aria-label="${bagCategoryLabels[category]}・${item.name}"><img src="${item.image}" alt="${item.name}"><b>${formatLargeNumber(state[item.count])}</b><small>${item.name}</small></button>`}).join('');
    const emptySlots=Array.from({length:Math.max(0,capacity-used)},()=>'<span></span>').join('');
    inner.innerHTML=`<section class="bag-rank-panel"><div><small>儲物袋品階</small><b>${bagRankNames[state.bagRank-1]}階</b><span>已用 ${used} / ${capacity} 格</span></div><img src="assets/qstyle-v2/mending-silk-cutout.png" alt="補天絲"><div><small>持有補天絲</small><b>${formatLargeNumber(state.mendingSilk||0)}</b>${state.bagRank<16?`<span>升階需要 ${formatLargeNumber(cost)}</span>`:'<span>已達最高品階</span>'}</div>${state.bagRank<16?`<button id="upgradeBagBtn" ${(state.mendingSilk||0)>=cost?'':'disabled'}>升至${bagRankNames[state.bagRank]}階</button>`:'<strong>十六階</strong>'}</section><div class="bag-organize-bar"><button id="organizeBagBtn">${state.bagSortMode==='category'?'重新整理':'整理'}</button><small>資源道具 → 裝備 → 功法秘訣 → 丹藥靈釀 → 製作素材 → 信物特殊</small></div><div class="inventory-grid ${state.bagSortMode==='category'?'organized':''}">${itemButtons}${emptySlots}</div><small class="empty-note">${used?'點擊道具可查看詳細資訊':'目前儲物袋空空如也'}</small>`;
    $$('[data-bag-item]').forEach(button=>button.onclick=()=>openItemModal(button.dataset.bagItem));$('#organizeBagBtn').onclick=organizeBag;if(state.bagRank<16)$('#upgradeBagBtn').onclick=upgradeBag;
    return;
  }
