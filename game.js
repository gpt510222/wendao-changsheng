...
function immortalLeaderboardKey(){const a=normalizeAscension();if(!a.ascended)return '';const im=a.immortalRealm,exploration=Math.min(100,(Array.isArray(im.investigated)?im.investigated.length:0)*15+(im.guideStoneStage||0)*10+(im.outerRouteCleared?25:0));return `${Math.max(0,im.guideStoneStage||0)}:${exploration}:${Math.max(0,im.expedition?.runs||0)}`}
...
async function fetchAscensionLeaderboard(){const query='select=user_id,player_name,spirit_level,sword_level,body_level,game_version&limit=1000',response=await fetch(`${leaderboardConfig.url}/rest/v1/player_rankings?${query}`,{headers:leaderboardHeaders()});if(!response.ok)throw new Error('ascension ranking fetch failed');return (await response.json()).map(parseAscensionLeaderboardRecord).filter(Boolean).sort((a,b)=>a.ascendedAt-b.ascendedAt).slice(0,leaderboardConfig.limit)}
async function fetchImmortalLeaderboard(){const query='select=user_id,player_name,game_version,updated_at&limit=1000',response=await fetch(`${leaderboardConfig.url}/rest/v1/player_rankings?${query}`,{headers:leaderboardHeaders()});if(!response.ok)throw new Error('immortal ranking fetch failed');return (await response.json()).map(parseImmortalLeaderboardRecord).filter(Boolean).sort((a,b)=>b.restorationStage-a.restorationStage||b.exploration-a.exploration||b.expeditionRuns-a.expeditionRuns||String(a.updated_at).localeCompare(String(b.updated_at))).slice(0,leaderboardConfig.limit)}
...
function immortalState(){const im=normalizeAscension().immortalRealm;if(!im.inventoryVersion){state.immortalResidualSourceCount=Math.max(0,Math.floor(Number(state.immortalResidualSourceCount)||0))+Math.max(0,Math.floor(Number(im.residualSource)||0));state.immortalBarrenTraceCount=Math.max(0,Math.floor(Number(state.immortalBarrenTraceCount)||0))+Math.max(0,Math.floor(Number(im.barrenTraces)||0));im.inventoryVersion=1}if(!im.storyFlags||typeof im.storyFlags!=='object')im.storyFlags={};Object.defineProperties(im,{residualSource:{configurable:true,enumerable:true,get:()=>state.immortalResidualSourceCount||0,set:value=>state.immortalResidualSourceCount=Math.max(0,Math.floor(Number(value)||0))},barrenTraces:{configurable:true,enumerable:true,get:()=>state.immortalBarrenTraceCount||0,set:value=>state.immortalBarrenTraceCount=Math.max(0,Math.floor(Number(value)||0))}});return im}
const immortalExpeditionDailyLimit=3,immortalRestorationDurations=[4*3600000,20*3600000,48*3600000];
...
function immortalAdaptation(){return [0,3,6,10][Math.max(0,Math.min(3,immortalState().guideStoneStage||0))]}
...
function immortalHeader(eyebrow,title,description){const im=immortalState(),storyCount=Object.values(im.storyFlags||{}).filter(Boolean).length;return `<header class="immortal-page-head"><small>${eyebrow}</small><h2>${title}</h2><p>${description}</p><div class="immortal-resource-row"><span>仙界歷程 <b>第 ${immortalTimelineDay()} 日</b></span><span>殘界源質 <b>${im.residualSource}</b></span><span>荒痕 <b>${im.barrenTraces}</b></span><span>探索 <b>${immortalExploration()}%</b></span><span>古臺碑錄 <b>${storyCount} / 7</b></span></div></header>`}
function playImmortalStory({chapter,title,lines,onDone}){const modal=ensureAscensionModal();modal.classList.add('show');playAscensionStory({theme:'immortalBarren',chapter,title,background:'assets/qstyle-v2/ascension/immortal-realm-barren-v1.png',className:'immortal-arrival',lines,onDone:()=>{modal.classList.remove('show');onDone?.();save();render();resumeWorldBgm();queueImmortalMilestoneStory()}})}
...
function renderImmortalFeature(page){if(page==='immortal-domain'){renderImmortalDomain();renderImmortalChronicle()}else if(page==='immortal-expedition')renderImmortalExpedition();else if(page==='immortal-path')renderImmortalPath();else if(page==='immortal-restoration')renderImmortalRestoration();queueImmortalMilestoneStory()}
function renderImmortalDomain(){const im=immortalState(),done=id=>im.investigated.includes(id);$('#featureDescription').innerHTML=`<section class="immortal-play-page">${immortalHeader('荒蕪仙界・第一仙域','飛升古臺','仙界沒有靈氣。先讀懂這座殘臺留下的痕跡，才知道路斷在何處。')}<div class="immortal-objective"><small>目前目標</small><b>${!done('array')?'調查熄滅古陣':!done('stele')?'調查無字殘碑':!im.firstBattleWon?'清除界門下的枯靈殘兵':!im.outerRouteCleared?'由遠征深入飛升古臺外圍':'修復引道碑，尋找無息荒原的方向'}</b></div><div class="immortal-card-grid">${Object.entries(immortalInvestigations).map(([id,item],index)=>`<article class="immortal-action-card ${done(id)?'complete':''}"><i>0${index+1}</i><div><b>${item.name}</b><p>${item.description}</p></div><button data-investigate="${id}" ${id==='gate'&&!done('array')&&!done('stele')?'disabled':''}>${done(id)?'重看痕跡':id==='gate'?'接近界門':'調查'}</button></article>`).join('')}</div><aside class="immortal-next-region ${im.outerRouteCleared&&im.guideStoneStage>=3?'revealed':''}"><small>下一仙域</small><b>無息荒原</b><span>${im.outerRouteCleared&&im.guideStoneStage>=3?'引道碑已指向荒原，但更深處仍未開放。':'迷霧封鎖・需完成外圍遠征並修復引道碑'}</span></aside></section>`;$$('[data-investigate]').forEach(button=>button.onclick=()=>investigateImmortalSite(button.dataset.investigate))}
...
function renderImmortalPath(){const ratio=immortalSuppression(),adapt=immortalAdaptation();$('#featureDescription').innerHTML=`<section class="immortal-play-page">${immortalHeader('凡道為基・仙途初啟','界應','界應代表你與仙界法則的適應程度。它不覆蓋凡間成長，只改變凡間力量在仙界能發揮多少。')}<div class="immortal-power"><small>凡間原始戰力</small><b>${formatCombatPower(combatPower())}</b><i>仙界實效戰力</i><strong>${formatCombatPower(immortalEffectiveCombatPower())}</strong></div><div class="immortal-adaptation"><div><span>界應</span><b>${adapt} / 100</b></div><progress max="100" value="${adapt}"></progress><p>${adapt?'引道碑已讓你開始被這片天地承認。':'仙界排斥所有來自凡間的既有法則。修復引道碑後，可取得最初的界應。'}</p></div><div class="immortal-ratio-grid">${[['攻擊','attack'],['防禦','defense'],['氣血','hp'],['命中','accuracy'],['閃避','evasion']].map(([label,key])=>`<span>${label}<b>${Math.round(ratio[key]*100)}%</b></span>`).join('')}</div><aside class="immortal-rule-note"><b>壓制規則</b><p>只作用於仙界戰鬥。凡間面板、修行成果與排行榜戰力完全保留；日後凡間三線仍可成長，返回仙界後再按界應比例換算。</p><small>長期目標・界應 100 → 仙界壓制完全解除 → 尋找破碎虛空之法</small></aside></section>`}
...
function bagItemSortProfile(key,item){
  ...
  else if(key.startsWith('main-material-')||key.startsWith('craft-material-')||item.brewBase||item.immortalMaterial||key==='mendingSilk'){category=4}
...
function renderImmortalHelp(tab='immortal-overview'){const pages={
  ...
  'immortal-domain':helpCard('仙域探索',['仙域顯示目前所在區域、探索進度與可調查地點。','飛升古臺共有熄滅古陣、無字殘碑與傾塌界門三處初始調查點；部分地點有前置線索或戰鬥。','調查、首次戰鬥、遠征與復界共同提高區域探索進度。'])+helpCard('荒界素材',['殘界源質是崩毀世界留下的法則殘質，主要用於復界與仙域建設。','荒痕來自枯靈及荒蕪遺址，是修復高階設施的關鍵素材。','兩種素材目前不可直接使用、不可出售；需要在復界工程中投入。']),
  'immortal-expedition':helpCard('每日遠征',['飛升古臺外圍採固定五節點路線，包含探索、採集、戰鬥與撤離。','每日可開始3次遠征，開始時消耗一次額度，現實00:00恢復。','已開始的遠征可跨日繼續，不會因刷新而中斷或再次扣除。','戰鬥失敗不會直接清除遠征進度，可提升凡間能力或界應後再次挑戰。'])+helpCard('遠征收穫',['枯骨石林與廢棄營地可取得殘界源質；擊敗枯靈殘兵可取得荒痕。','完成整條外圍路線會記錄遠征次數，並發現通往無息荒原的線索。']),
  'immortal-path':helpCard('界應',['界應代表角色與仙界法則的適應程度，範圍為0至100。','界應不會取代凡間三條成長線；凡間能力仍可繼續提升，進入仙界後再按壓制比例換算。','修復引道碑可取得最初10點界應；後續仙域將提供更多提升途徑。'])+helpCard('仙界戰力壓制',['界應為0時，攻擊發揮50%、防禦60%、氣血70%、命中與閃避80%。','界應提高後，各項比例會逐步回復；達到100時完全解除仙界壓制。','狀態欄與仙途頁顯示仙界實效戰力；凡間人物戰力與潛龍榜數值不受影響。']),
  'immortal-restoration':helpCard('復界工程',['飛升古臺目前可修復引道碑，共分辨認碑基、接續斷紋、喚醒引道三階。','三階施工時間依序為現實4小時、20小時與48小時，同一時間只能進行一項工程。','材料在開工時立即扣除；完成後必須回到復界頁領取，才會提升修復階段及界應。'])+helpCard('引道碑用途',['引道碑會將飛升者的存在寫入仙界法則，逐步緩解戰力壓制。','完成引道碑並通過古臺外圍遠征後，才能確認下一仙域「無息荒原」的入口。']),
...
setInterval(()=>{if(currentFeature==='immortal-restoration'&&immortalRestorationJob())renderImmortalRestoration()},5000);
