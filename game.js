...
function playAscensionStory({theme,chapter,title,background,className='',lines,onDone}){startBgm(theme);const c=$('#ascensionContent');let step=0;const draw=()=>{const line=lines[step];if(!line){onDone?.();return}const choices=line.choices||[],body=choices.length?`<div class="ascension-story-choices">${choices.map((choice,index)=>`<button data-story-choice="${index}">${choice.label}</button>`).join('')}</div>`:'<button class="ascension-story-next" data-story-next>繼續</button>';const portrait=line.portrait==='none'?'':(ascensionArt[line.portrait]||ascensionArt.player)();c.innerHTML=ascensionDialogueStage({chapter,title,background,portrait,speaker:line.speaker,text:line.text,body,className});if(choices.length)c.querySelectorAll('[data-story-choice]').forEach(button=>button.onclick=()=>{const choice=choices[+button.dataset.storyChoice];choice.onSelect?.();if(choice.followup?.length)lines.splice(step+1,0,...choice.followup);step++;draw()});else c.querySelector('[data-story-next]').onclick=()=>{step++;draw()}};draw()}
