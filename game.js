...
function immortalState(){return normalizeAscension().immortalRealm}
...
function renderImmortalRestoration(){const im=immortalState(),stage=im.guideStoneStage;$('#featureDescription').innerHTML=`<section class="immortal-play-page">${immortalHeader('殘界待甦・第一修復','引道碑','修復不是把仙界變回從前，而是讓已經斷裂的世界重新有路可走。')}<div class="guide-stone-visual stage-${stage}"><i></i><b>${stage>=3?'引道微明':stage?'殘紋復燃':'死寂碑身'}</b><span>${stage} / 3</span></div><div class="restoration-stages">${immortalRestorationStages.map((item,index)=>`<article class="${stage>index?'complete':stage===index?'current':''}"><i>${stage>index?'✓':index+1}</i><div><b>${item.name}</b><span>${item.cost}</span></div>${stage===index?`<button data-restore-stage="${index}">修復</button>`:''}</article>`).join('')}</div>${stage>=3?'<div class="immortal-complete-note"><b>界應 +10</b><span>引道碑已將你的存在寫入這片天地。無息荒原的方位若隱若現。</span></div>':'<p class="immortal-hint">所需的仙界材料可由仙域調查與飛升古臺外圍遠征取得。</p>'}</section>`;$('#featureDescription').querySelector('[data-restore-stage]')?.addEventListener('click',restoreGuideStone)}
