...
const startingTechniques=[
  ...
  {id:'body-origin',name:'沉肩震步',kind:'body',min:.9,max:1.05,guardBonus:.05,description:'沉肩踏地，以命骨與玄軀推動全身，近身震開對手。'}
];
...
const bodyInjuries={
  scratch:{name:'擦傷',severity:1,duration:900000,description:'鍛體所得淬鍊度降低10%，但不會中斷修行。'},
...
const bodyRealmPassives=[
  {realm:'塵軀',name:'凡軀初煉',description:'開啟體力、淬鍊度與鍛體修行。'},
  ...
  {realm:'鎮陸',name:'鎮陸真軀',description:'滿足凡界煉體飛升境界條件。'},
...
const mainlineMaterialDescriptions={xuansi:'由主副本中受地脈靈氣浸染的玄蠶絲抽理而成，絲質輕韌且容易承載細小陣紋，是製作冠的主要素材。',xuanjuan:'將多股玄靈絲線反覆浸潤、織成的柔韌絹材，常見於古觀與守界司遺留物資，是製作法衣的主要素材。',xuanpi:'取自副本內受靈氣淬鍊的妖獸表皮，經去除妖煞並壓入玄紋後耐磨不裂，是製作護腕的主要素材。',lingpi:'由靈獸皮革以地脈靈液鞣製而成，兼具柔韌與穩固靈力的特性，是製作腰帶的主要素材。',fengpi:'取自擅長御風的妖獸或長年受天風侵蝕的靈皮，質地輕盈、能疏導身法靈氣，是製作靴履的主要素材。',lingyu:'由副本靈脈與古老陣眼附近凝結的溫潤玉料，能安定佩戴者氣機並保存細微道韻，是製作玉佩的主要素材。',lingjing:'在高壓地脈、界壁碎片與九鎖靈流交會處形成的透明晶體，質地堅硬且便於凝聚術式，是製作指環的主要素材。'};
...
defaults.sectSearchAvailableAt=0;
  defaults.mainlineCleared=0;defaults.mainlineStories={};defaults.mainlineMaterials={};defaults.mainlineLoot={};defaults.mainlineHarvest=[];defaults.mainlineSpiritStoneBag=0;defaults.mainlineWoodBag=0;defaults.mainlineIronBag=0;defaults.mainlineFoodBag=0;
...
function pathResourceLine(path){if(path==='spirit')return `修為 ${formatLargeNumber(state.free)} / ${formatLargeNumber(req(state.spiritLevel))}`;if(path==='sword')return `劍元 ${formatLargeNumber(state.swordEssence)} / ${formatLargeNumber(swordReq(state.swordLevel||0))}`;return `體力 ${Math.floor(state.bodyStamina)} / 100・淬鍊度 ${formatLargeNumber(state.bodyTemper)} / ${formatLargeNumber(bodyTemperNeed())}`}
function primaryPathAction(){const path=state.activePath||state.firstPath||'spirit';if(path==='spirit')return upgrade('spirit');if(path==='sword'){if(!state.swordEmbryo)return openPrimarySwordView('sword');return upgrade('sword')}openPrimaryBodyView(state.bodyTemper>=bodyTemperNeed()?'body':'training')}
function renderPrimarySanctum(){
  ...
  else{refreshBodyState();resourceLabel.textContent='當前淬鍊度';$('#totalQi').textContent=`${formatLargeNumber(state.bodyTemper)} / ${formatLargeNumber(bodyTemperNeed())}`;rateLabel.textContent='肉身狀態';$('#rateText').textContent=`體力 ${Math.floor(state.bodyStamina)} / 100`}
}
...
function bodyTrainingOptions(){const realm=Math.floor((state.bodyLevel||0)/10)+1,need=bodyTemperNeed(),injuryReduction=bodyPassiveUnlocked(3)?10:0,staminaFactor=activeBodyInjury()==='internal'?1.25:1,load=Math.floor(state.bodyTrainingLoad||0);return {
  basic:{name:'基礎鍛體',stamina:Math.ceil(10*staminaFactor),food:10*realm,wood:0,stone:0,gain:bodyTemperGain(need*.04),risk:0,load:5,description:'安全打磨根基；材料需求最低，即使帶傷也能持續修行。'},
  bath:{name:'藥浴鍛體',stamina:Math.ceil(20*staminaFactor),food:30*realm,wood:18*realm,stone:0,gain:bodyTemperGain(need*.11),risk:0,load:-25,description:'以食物與木材調製藥浴，高效鍛體並舒緩負荷與傷勢。'},
  extreme:{name:'極限鍛體',stamina:Math.ceil(30*staminaFactor),food:50*realm,wood:0,stone:0,gain:bodyTemperGain(need*.17),risk:Math.max(5,8+Math.floor(load*.22)-injuryReduction),load:25,description:'收益最高；肉身負荷越重，留下傷勢的機率越高。'}
}}
...
function bodyTrainingCapacity(option){const remaining=Math.max(0,bodyTemperNeed()-state.bodyTemper);return Math.max(0,Math.min(Math.floor(state.bodyStamina/option.stamina),Math.floor(state.food/option.food),option.wood?Math.floor(state.wood/option.wood):Infinity,option.stone?Math.floor(state.spiritStone/option.stone):Infinity,Math.ceil(remaining/Math.max(1,option.gain))))}
function trainBody(kind,batch=false){
  refreshBodyState();if(!state.bodyPathOpened)return toast('尚未開啟煉體之路');if(state.bodyLevel>=maxBodyLevel)return toast('肉身境界已圓滿');let option=bodyTrainingOptions()[kind];if(!option)return;if(kind==='extreme'&&activeBodyInjury()==='tendon')return toast('筋傷未癒，無法進行極限鍛體');let times=batch?bodyTrainingCapacity(option):Math.min(1,bodyTrainingCapacity(option));if(times<1)return toast(state.bodyTemper>=bodyTemperNeed()?'淬鍊度已足，請先完成突破':'體力或鍛體材料不足');let totalGain=0,completed=0,newInjury='';
  while(completed<times&&state.bodyTemper<bodyTemperNeed()){
    option=bodyTrainingOptions()[kind];if(kind==='extreme'&&activeBodyInjury()==='tendon')break;if(state.bodyStamina<option.stamina||state.food<option.food||state.wood<option.wood||state.spiritStone<option.stone)break;state.bodyStamina-=option.stamina;state.food-=option.food;state.wood-=option.wood;state.spiritStone-=option.stone;state.bodyTemper+=option.gain;totalGain+=option.gain;completed++;state.bodyTrainingLoad=Math.max(0,Math.min(100,(state.bodyTrainingLoad||0)+option.load));if(kind==='bath'){if(activeBodyInjury()==='scratch'){state.bodyInjury='';state.bodyInjuryUntil=0}else if(activeBodyInjury()==='internal')state.bodyInjuryUntil=Math.max(gameNow(),state.bodyInjuryUntil-600000)}if(option.risk&&Math.random()*100<option.risk){const roll=Math.random();inflictBodyInjury(roll<.5?'scratch':roll<.82?'internal':'tendon');newInjury=bodyInjuries[state.bodyInjury].name}}
  state.bodyStaminaUpdatedAt=gameNow();state.bodyTrainingLoadUpdatedAt=gameNow();toast(`${option.name}完成 ${completed} 次・淬鍊度+${formatLargeNumber(totalGain)}${newInjury?`・留下${newInjury}`:''}`);renderBodyDestination('training');render();save();
}
...
function effectiveCore(attribute){const total=(state[attribute]||0)+artBonusFor(attribute)+swordPathBonus(attribute)+equippedAttributeBonus(attribute);return attribute==='agility'&&activeBodyInjury()==='tendon'?total*.85:total}
...
function swordEssenceRate(){return state.swordPathOpened?Math.max(1,Math.floor(12.5*pathEfficiency(state.swordLevel||0))):0}
...
function load() {
  try {
    ...
    if(old) { state={...defaults,...old,free:(old.free||0)+(old.spiritQi||0)+(old.bodyQi||0),tribPill1:Math.max(0,old.pills||0),tribulationPillMigration:1};delete state.pills;delete state.sectTokens;delete state.sectTokenDaily;state.bornAt ||= Date.now(); }
  } catch {}
}
...
function render() {
  const spiritMax=state.spiritLevel>=maxSpiritLevel,swordMax=(state.swordLevel||0)>=maxSwordLevel,spiritCost=spiritMax?null:req(state.spiritLevel),swordCost=swordMax?null:swordReq(state.swordLevel||0),free=state.free,swordEssence=state.swordEssence;
...
async function openCultivationPath(type){
  ...
  if(type==='spirit'){
    ...
    state[cost.key]-=cost.amount;state[key]=true;if(type==='body'&&!(state.swordMoves||[]).length)state.swordMoves=['body-origin'];toast(`已開啟${meta.name}之路・${meta.realm}`);
  }
...
function tribulate() {
  ...
  scheduleTribulation(()=>{
    ...
    if(success) {
      ...
      state.free=state.free>(cost+1n)/2n?state.free-(cost+1n)/2n:0n;
...
async function startSwordBreakthrough(cost=swordReq(state.swordLevel||0)){
  if(tribulationLocked)return;const next=(state.swordLevel||0)+1;if(next%10!==0||(state.swordTrialWins||0)<next)return toast(`需先通過試劍境第 ${next} 關`);if(state.swordEssence<cost)return toast(`尚缺 ${formatLargeNumber(cost-state.swordEssence)} 劍元`);
...
function swordTrialPower(stage){const milestone=stage%10===0,progress=Math.max(0,Math.min(maxSwordLevel,stage-1))/maxSwordLevel,factor=milestone?1.15+progress*.1:.78+progress*.12;return Math.ceil((swordTrialReferencePower(stage)*factor)/5)*5}
...
function combatTechniqueById(id){return startingTechniques.find(move=>move.id===id)||swordTechniqueCatalog.find(move=>move.id===id)}
function combatTechniqueAvailable(move){return !!move&&(move.id==='origin'?!!state.spiritPathOpened:move.id==='body-origin'?!!state.bodyPathOpened:swordTechniqueUnlocked(move.id))}
...
function setSwordMove(id,slot){
  const move=combatTechniqueById(id);if(!move||slot<0||slot>1)return;if(move.embryo&&move.embryo!==state.swordEmbryo)return toast(`此招只屬於${swordEmbryos[move.embryo].name}，目前尚未掌握`);if(!combatTechniqueAvailable(move))return toast(`通過試劍境第${swordTechniqueUnlockStage(id)}關後解鎖`);state.swordMoves=[...(state.swordMoves||['origin'])];const other=slot===0?1:0;if(state.swordMoves[other]===id){const displaced=state.swordMoves[slot];state.swordMoves[slot]=id;state.swordMoves[other]=displaced;toast(`${move.name}已調整為第${slot+1}式`)}else{state.swordMoves[slot]=id;toast(`${move.name}已設為第${slot+1}式`)}renderArtsPanel('moves');save();
}
...
function swordProgressCard(){
  const level=state.swordLevel||0,maxed=level>=maxSwordLevel,cost=maxed?null:swordReq(level),next=level+1,trialRequired=next%10===0&&(state.swordTrialWins||0)<next,ready=!maxed&&!trialRequired&&state.swordEssence>=cost;
...
function renderBodyExperienceView(view,inner){
  refreshBodyState();const need=bodyTemperNeed(),injuryId=activeBodyInjury(),injury=bodyInjuries[injuryId],nextIsRealm=(state.bodyLevel+1)%10===0;
  ...
  if(state.bodyLevel>=maxBodyLevel){inner.innerHTML='<div class="realm-lock"><b>無量・十層</b><small>凡軀體系已圓滿，淬鍊度與鍛體操作已停止；後續將銜接上位面肉身體系。</small></div>';return}
  if(view==='body'){
    const heal=bodyHealCost(injuryId);inner.innerHTML=`<section class="body-dashboard"><div class="body-seal">體</div><div><small>當前肉身</small><h2>${realmName(state.bodyLevel,bodyRealms)}</h2><p>先以鍛體累積淬鍊度；小層可直接突破，大境界必須撐過肉身試煉。</p></div><div class="body-meter"><span>體力 <b>${Math.floor(state.bodyStamina)} / 100</b></span><i><em style="width:${Math.min(100,state.bodyStamina)}%"></em></i><small>每1分鐘恢復1點・靈藥可使體力溢出</small></div><div class="body-meter temper"><span>淬鍊度 <b>${formatLargeNumber(state.bodyTemper)} / ${formatLargeNumber(need)}</b></span><i><em style="width:${Math.min(100,state.bodyTemper/need*100)}%"></em></i></div><div class="body-meter body-load"><span>肉身負荷 <b>${Math.floor(state.bodyTrainingLoad)} / 100</b></span><i><em style="width:${state.bodyTrainingLoad}%"></em></i><small>每1分鐘自然消退1點・負荷越高，極限鍛體風險越大</small></div><div class="body-condition ${injury?'injured':''}"><b>${injury?injury.name:'肉身無傷'}</b><small>${injury?`${injury.description}・剩餘 ${formatDuration(state.bodyInjuryUntil-gameNow())}`:'目前可進行所有鍛體方式。'}</small>${injury?`<button id="healBodyBtn">療傷・食物 ${formatLargeNumber(heal.food)}／木材 ${formatLargeNumber(heal.wood)}／靈石 ${formatLargeNumber(heal.stone)}</button>`:''}</div><button id="goBodyAction" class="jade-button">${state.bodyTemper<need?'前往鍛體':nextIsRealm?'前往肉身試煉':`突破至${realmName(state.bodyLevel+1,bodyRealms)}`}</button></section>`;if(injury)$('#healBodyBtn').onclick=healBodyInjury;$('#goBodyAction').onclick=()=>state.bodyTemper<need?renderBodyDestination('training'):nextIsRealm?renderBodyDestination('bodyTrial'):upgrade('body');return;
  }
  if(view==='training'){
    const options=bodyTrainingOptions();inner.innerHTML=`<section class="body-training-head"><h2>鍛體場</h2><p>安全鍛體維持進度，藥浴可卸除負荷並舒緩傷勢，極限鍛體則以風險換取最高收益。</p><div class="body-training-resources" aria-label="鍛體狀態與材料庫存"><span class="stamina-resource"><i class="body-resource-seal">體</i><small>體力</small><b>${Math.floor(state.bodyStamina)} / 100</b><em>每1分鐘恢復1點體力</em></span><span><i class="body-resource-seal temper">煉</i><small>淬鍊度</small><b>${formatLargeNumber(state.bodyTemper)} / ${formatLargeNumber(need)}</b></span><span><i class="body-resource-seal">負</i><small>肉身負荷</small><b>${Math.floor(state.bodyTrainingLoad)} / 100</b><em>每1分鐘消退1點</em></span><span><img src="assets/qstyle-v2/food-cutout.png" alt=""><small>食物</small><b>${formatLargeNumber(state.food)}</b></span><span><img src="assets/qstyle-v2/wood-cutout.png" alt=""><small>木材</small><b>${formatLargeNumber(state.wood)}</b></span><span><img src="assets/qstyle-v2/spirit-stone.png" alt=""><small>靈石</small><b>${formatLargeNumber(state.spiritStone)}</b></span></div></section><div class="body-training-grid">${Object.entries(options).map(([id,item])=>{const capacity=bodyTrainingCapacity(item),disabled=state.bodyStamina<item.stamina||state.food<item.food||state.wood<item.wood||state.spiritStone<item.stone||id==='extreme'&&injuryId==='tendon';return `<article><h3>${item.name}</h3><p>${item.description}</p><small>消耗：體力 ${item.stamina}・食物 ${formatLargeNumber(item.food)}${item.wood?`・木材 ${formatLargeNumber(item.wood)}`:''}${item.stone?`・靈石 ${formatLargeNumber(item.stone)}`:''}</small><b>淬鍊度 +${formatLargeNumber(item.gain)}・負荷 ${item.load>0?'+':''}${item.load}${item.risk?`・受傷率 ${item.risk}%`:''}</b><div class="body-training-actions"><button data-body-training="${id}" ${disabled?'disabled':''}>開始鍛體</button><button data-body-training-batch="${id}" ${capacity<1||id==='extreme'&&injuryId==='tendon'?'disabled':''}>批量鍛體・${capacity}次</button></div></article>`}).join('')}</div>`;$$('[data-body-training]').forEach(button=>button.onclick=()=>trainBody(button.dataset.bodyTraining));$$('[data-body-training-batch]').forEach(button=>button.onclick=()=>trainBody(button.dataset.bodyTrainingBatch,true));return;
  }
  const target=5+Math.floor((state.bodyLevel+1)/20);inner.innerHTML=`<section class="body-trial-card"><div class="trial-orb body-orb">守</div><h2>${nextIsRealm?`${realmName(state.bodyLevel+1,bodyRealms)}・肉身試煉`:'尚未抵達大境界關口'}</h2><p>${nextIsRealm?`不求擊倒試煉化身，只需在猛烈攻勢下撐過 ${target} 回合。失敗將留下傷勢，但可再次挑戰。`:'此處僅用於每十層的大境界突破；小層請在肉身頁直接突破。'}</p><strong>${state.bodyTemper<need?`淬鍊度尚缺 ${formatLargeNumber(need-state.bodyTemper)}`:'已具備試煉資格'}</strong><button id="startBodyTrial" class="jade-button" ${!nextIsRealm||state.bodyTemper<need?'disabled':''}>承受試煉</button></section>`;$('#startBodyTrial').onclick=startBodyTrial;
}
function renderSwordPathSummary(){
  const inner=$('#experienceInner');if(!inner||!state.swordEmbryo||inner.querySelector('.sword-path-summary'))return;const counts=swordPathMarkCounts(),next=Math.max(10,Math.ceil(((state.swordLevel||0)+1)/10)*10),path=swordPathAlignment(next),need=swordPathExperienceNeed(next),total=Math.floor(state.righteousness+state.evilQi),anchor=inner.querySelector('.sword-dashboard,.sword-trial-card');if(!anchor)return;
...
function renderExperiencePanel(view='overview'){
  ...
  if(view==='spiritSide'){currentExperienceView=view;$('#featureDescription').innerHTML='<div class="experience-tabs"><button data-experience-view="overview">返回兼修</button></div><div id="experienceInner"></div>';const inner=$('#experienceInner'),cost=req(state.spiritLevel);inner.innerHTML=`<section class="sword-progress-card"><div><small>兼修練氣</small><h2>${realmName(state.spiritLevel,spiritRealms)}</h2><span>修為 ${formatLargeNumber(state.free)} / ${formatLargeNumber(cost)}</span><em>每5秒增長 ${formatLargeNumber(rate())} 修為</em></div><button id="advanceSideSpirit" class="jade-button" ${state.free>=cost?'':'disabled'}>突破練氣境界</button></section>`;$('#advanceSideSpirit').onclick=()=>upgrade('spirit');$('[data-experience-view="overview"]').onclick=()=>renderExperiencePanel('overview');return}
...
function openMainlineStory(id){if(!state.cultivationAwakened)return toast('完成新手教程後開啟九鎖封天');if(divineRoamingActive())return toast('神念遠遊期間無法進入副本・請先要求神念回歸');mainlineStoryStage=mortalMainline[id-1];mainlineStoryStep=0;startMainlineBgm(mainlineStoryStage);showMainlineDialogue()}
...
function grantMainlineDrops(stage){
  state.mainlineMaterials=state.mainlineMaterials||{};state.mainlineLoot=state.mainlineLoot||{};const odd=stage.id%2===1,results=[],addLoot=(name,amount=1)=>{state.mainlineLoot[name]=(state.mainlineLoot[name]||0)+amount;results.push(`${name} ×${amount}`)},roll=(chance,fn)=>Math.random()<chance&&fn(),range=(min,max)=>min+Math.floor(Math.random()*(max-min+1));
  const giveMain=()=>{const picked=mainlineMaterials[Math.floor(Math.random()*mainlineMaterials.length)],count=`mainlineMaterial_${picked[1]}`;state[count]=(state[count]||0)+1;results.push(`${picked[0]} ×1`)};giveMain();if(odd)roll(.35,giveMain);
  const tierName=tierMaterials[stage.realm-1];roll(odd?.3:.75,()=>addLoot(tierName));if(!odd)roll(.15,()=>addLoot(tierName));
  const bias=stage.id>=17?Math.floor(Math.random()*4):(stage.id-1)%4,pickBias=()=>Math.random()<.4?bias:[0,1,2,3].filter(x=>x!==bias)[Math.floor(Math.random()*3)],herb=mainlineBias[pickBias()];roll(odd?.55:.4,()=>addLoot(herb[1]));if(odd)roll(.15,()=>addLoot(mainlineBias[pickBias()][1]));roll(odd?.55:.4,()=>addLoot(mainlineBias[pickBias()][2]));if(odd)roll(.15,()=>addLoot(mainlineBias[pickBias()][2]));
  roll(stage.id===17?.45:stage.id===18?.7:odd?.3:.6,()=>addLoot('丹砂'));addLoot(`${spiritRealms[stage.realm-1]}丹`,range(1,2));
...
function sectTechniqueSet(sect=state.sect,star=state.sectStar){
  ...
  return picked.map((kind,index)=>{const element=artElements[(seed+index*star+index)%artElements.length][0],name=`${clean}${suffix[kind]}`;return{id:`${sect}-${index}`,sourceSect:sect,name,kind,element,tier:star,level:1,slot:index,sectTechniqueVersion:2}})
}
...
function normalizeLearnedArts(){state.learnedArts=(state.learnedArts||[]).map(art=>{if(!art.sourceSect)return art;const expected=sectTechniqueReplacement(art),level=Math.max(1,Math.min(10,art.level||1));if(!expected)return {...art,level};if((art.sectTechniqueVersion||1)<2&&sectTechniqueChanged(art,expected))return {...art,level,legacySectTechnique:true,sectTechniqueVersion:1};return {...expected,level,legacySectTechnique:false}});state.learnedBookIds=Array.from(new Set([...(state.learnedBookIds||[]),...state.learnedArts.filter(art=>art.source==='book').map(art=>art.id)]))}
...
function renderSwordMoves(inner){
  const equipped=(state.swordMoves||[]).map(combatTechniqueById),known=[...startingTechniques,...swordTechniqueCatalog].filter(combatTechniqueAvailable);
  const moveCard=move=>`<article class="origin-move-card ${move.kind||'spirit'}-move-card"><header><b>${move.name}</b><em>${move.kind==='body'?'煉體架式':move.embryo?`${swordEmbryos[move.embryo].short}劍招`:'練氣入道招'}</em></header><p>${move.description}</p><strong>傷害 ${Math.round(move.min*100)}%～${Math.round(move.max*100)}%${move.kind==='body'?'・以根骨與體魄出力':''}</strong><small>已掌握・可跨系自由裝配</small><div><button data-equip-move="${move.id}" data-slot="0" ${state.swordMoves?.[0]===move.id?'disabled':''}>設為第一式</button><button data-equip-move="${move.id}" data-slot="1" ${state.swordMoves?.[1]===move.id?'disabled':''}>設為第二式</button></div></article>`;
...
function upgradeArt(id,view){const art=state.learnedArts.find(item=>item.id===id);if(!art||art.level>=10)return;const cost=artUpgradeCost(art);if(state.aura<cost)return toast('靈氣不足');state.aura-=cost;art.level++;toast(`${art.name}提升至${art.level}級`);renderArtsPanel(view,true);render();save()}
...
async function forgetArt(id,view='sect'){const index=state.learnedArts.findIndex(item=>item.id===id);if(index<0)return;const art=state.learnedArts[index],kind=artKinds[art.kind],lost=artTotalEffect(art),sharp=artSecondarySpiritualPower(art);if(!await gameConfirm(`確定遺忘「${art.name}」？\n將失去 ${kind.label}+${lost.toLocaleString()}${sharp?`、銳識+${sharp.toLocaleString()}`:''}，已投入的靈氣不會返還。`,{title:'遺忘功法',confirmText:'確認遺忘',danger:true}))return;state.learnedArts.splice(index,1);toast(`已遺忘${art.name}・${kind.label}-${lost}${sharp?`・銳識-${sharp}`:''}`);renderArtsPanel(view);render();save()}
...
function renderSectLearning(){
  ...
  $('#sectInner').innerHTML=`<section class="sect-learning"><h2>${state.sect}・傳功殿</h2><p class="sect-note">門派功法統一於傳功殿受授，職位越高，可學習的傳承越多。</p><div class="learning-list">${techniques.map((art,index)=>{const known=learned.some(item=>item.id===art.id),rankReady=index<limit,full=sectLearned.length>=state.artsCapacity,tier=['一','二','三','四','五','六','七','八','九'][art.tier-1],rare=art.kind==='ultimate'||art.kind==='fragment',concealed=rare&&!['confirmed','obtained'].includes(record?.discovery);if(concealed)return `<article class="sect-art-concealed"><b>封存傳承</b><span>傳功殿深處似有異樣，需先在本門累積功勳、晉升內門方可查證。</span><button disabled>尚未查明</button></article>`;return `<article><b>${tier}階・${art.name}</b><span>${artElements.find(([key])=>key===art.element)[1]}系・${artKinds[art.kind].tab}・${artKinds[art.kind].label}+${artBaseEffect(art)}</span><button data-learn-art="${art.id}" ${known||!rankReady||full?'disabled':''}>${known?'已習得':!rankReady?`需${index===1?'內門弟子':'親傳弟子'}`:full?'技能欄已滿':'學習'}</button></article>`}).join('')}</div></section>`;
...
function sectRecordTemplate(meta={}){return {name:meta.name||'',faction:meta.faction||'',star:Math.max(0,Math.floor(meta.star||0)),visited:true,merit:0,contribution:0,rank:0,task:'',actingLeader:false,npcAffinity:{},npcDaily:{},lastGreetingDay:'',lastSalaryDay:'',lastPracticeDay:'',discovery:'unknown',joinedAt:null,yearsProcessed:0}}
...
function sectRareTechnique(sect=state.sect,star=state.sectStar){return sectTechniqueSet(sect,star).find(art=>art.kind==='ultimate'||art.kind==='fragment')||null}
...
async function leaveSect(){
  if(!await gameConfirm(`確定脫離${state.sect}？\n本門功勳、貢獻、職位、好感與傳承見聞都會保留；免費尋訪其他門派需等待 3 個修練年。`,{title:'脫離門派',confirmText:'確認脫離',danger:true}))return;
  processSectYears();syncCurrentSectRecord();state.sectSearchAvailableAt=gameNow()+3*900000;
  state.sect='';state.sectFaction='';state.sectStar=0;state.sectMerit=0;state.sectContribution=0;state.sectRank=0;state.sectTask='';state.sectJoinedAt=null;state.sectYearsProcessed=0;state.actingLeader=false;state.npcAffinity={};state.npcDaily={};state.lastGreetingDay='';state.lastSalaryDay='';state.lastPracticeDay='';toast('已脫離門派・原門派進度已保留');render();renderSectPanel('home');save();
}
...
function renderSectShop(){
  ...
  inner.innerHTML=`<section class="sect-shop"><div class="shop-heading"><small>門派設施・物資兌換</small><h2>功勳堂</h2><span>門派貢獻 ${formatLargeNumber(state.sectContribution)}</span></div><div class="sect-shop-empty"><b>功勳堂正在重新整理</b><p>新的物資兌換項目將配合本門功勳與貢獻制度後續開放。</p></div></section>`;
}
...
function openItemModal(key){
  ...
  const showUse=item.usable&&!learned,canUse=showUse&&!sectBlocked&&!mindManualInvalid;const use=$('#itemModalUse');use.textContent=mindManualInvalid?'已失效':item.equipmentData?'裝備':'使用';use.classList.toggle('hidden',!showUse);use.disabled=!canUse||count<1;use.onclick=canUse?()=>useItem(key,itemModalQuantity):null;
  updateItemQuantity();
...
function battlePlayerStats(){
  ...
  const marks=swordPathMarkCounts(),guardPower=1+Math.min(.2,marks.righteous*.02);
  return {
    maxHp:Math.round(combatHealth(rootBone)*(activeBodyInjury()==='internal'?.85:1)),attack:Math.max(12,trueQi*5),qiAttack:Math.max(12,trueQi*5),bodyAttack:Math.max(12,rootBone*2+physique*3),defense:Math.max(0,physique*20)*guardPower,
    evasion:combatEvasion(agility),accuracy:combatAccuracy(spiritualPower),crit:Math.min(.5,combatCritical(spiritualPower)+marks.evil*.005)
  };
}
...
function bodyTechniqueRoll(attacker,defender,technique,multiplier){const bodyAccuracy=Math.max(attacker.accuracy||0,(effectiveCore('rootBone')+effectiveCore('physique'))*1.5),attacking={...attacker,attack:attacker.bodyAttack||attacker.attack,accuracy:bodyAccuracy},hit=damageRoll(attacking,defender,multiplier);if(!hit.dodged&&technique.guardBonus)battle.player.damageReduction=Math.max(battle.player.damageReduction||0,technique.guardBonus);return hit}
...
function enemyBattleTurn(){
  ...
  const damage=Math.max(hit.dodged?0:1,Math.round(hit.damage*(1-(battle.player.damageReduction||0))));battle.player.hp=Math.max(0,battle.player.hp-damage);animateBattleStrike('#enemySilhouette','#playerSilhouette',{...hit,damage},technique);
...
function finishBattle(won,reason){
  ...
  else if(battle.mode==='bodyTrial'){if(won){const need=bodyTemperNeed();if(state.bodyTemper>=need){state.bodyTemper-=need;state.bodyLevel++;applyAttributeGain(bodyAttributeGain(state.bodyLevel));reward=` 肉身突破至${realmName(state.bodyLevel,bodyRealms)}。`}else reward=' 試煉資格已失效，未完成突破。'}else{const injury=Math.random()<.6?'internal':'tendon';inflictBodyInjury(injury);reward=` 留下${bodyInjuries[injury].name}，療傷後可再次挑戰。`}}
  else if(battle.mode==='mainline'){if(won){const stage=battle.mainlineStage,first=stage.id===state.mainlineCleared+1;if(first)state.mainlineCleared=stage.id;reward=` ${first?mainlineAftermath[stage.id-1]:'你再次穩住此地殘存的靈流。'} 戰利品：${grantMainlineDrops(stage)}。`}else reward=' 可回刷素材、調整招式或提升任一修行道路後再次挑戰。'}
...
function runCaveFacilities(ticks){
  ...
  if(state.caveBodyEnabled&&state.bodyPathOpened&&state.bodyLevel<maxBodyLevel){state.caveBodyTicks+=ticks;const cycles=Math.floor(state.caveBodyTicks/720);if(cycles>0){state.caveBodyTicks-=cycles*720;const gain=Math.max(1,Math.floor(bodyTemperNeed()*(.0025+state.caveBodyLevel*.001)));state.bodyTemper+=gain*cycles}}
}
...
function renderHelp(tab='cultivation'){
  $$('.help-tabs button').forEach(button=>button.classList.toggle('active',button.dataset.helpTab===tab));const helpTabs=$('.help-tabs'),activeHelpTab=helpTabs?.querySelector('.active');if(helpTabs&&!helpTabs.dataset.horizontalScroll){helpTabs.dataset.horizontalScroll='true';helpTabs.addEventListener('wheel',event=>{if(Math.abs(event.deltaY)<=Math.abs(event.deltaX))return;helpTabs.scrollLeft+=event.deltaY;event.preventDefault()},{passive:false})}if(helpTabs&&activeHelpTab)helpTabs.scrollLeft=activeHelpTab.offsetLeft-helpTabs.offsetLeft-(helpTabs.clientWidth-activeHelpTab.clientWidth)/2;const pages={
    cultivation:helpCard('三路修行',['練氣消耗修為；淬劍消耗獨立累積的劍元；煉體消耗體力與物資並累積淬鍊度，三條路線可各自推進。','完成新手引導會正式開啟練氣；淬劍需消耗30隕鐵開啟，煉體需消耗120食物開啟。','修為與劍元可離線累積並精確保存；畫面會依數量自動切換萬、億等單位。','主介面會顯示各路線當前境界、進度與下一步入口。'])+helpCard('境界與功能解鎖',['三條路線皆為每個大境界十層；小層符合條件即可提升，大境界另有各自考驗。','練氣、淬劍或煉體任一路線達到要求，便可開放對應星級門派、坊市樓層、製作階級及九鎖封天關卡。','九鎖封天需依序通關；每個大境界有兩關，並要求三路之一達到相應境界。','意念入體訣可提前開啟人物屬性，未提前習得者會在化念一層自動領悟。','神念遠遊開啟後，可派遣神念自動探索已通關的九鎖封天關卡。'])+helpCard('練氣破境與雷劫',['練氣小層只需足夠修為；每逢第十層、跨入下一個大境界時才需渡雷劫。','可投入對應境界的渡劫丹，每顆提高5%成功率。','渡劫成功會提升境界與屬性；失敗會損失本次突破所需修為的一半。','渡劫演出結束後需手動離開結果畫面。'])+realmHelp(),
    sword:helpCard('本命劍與劍招',['開啟淬劍後可從重鋒、靈元、流影三種劍胚中擇一凝聚，目前選定後不能更換。','每種劍胚有兩招專屬劍招；第一招凝劍後可用，第二招於通過試劍境第20關後解鎖。','可在招式頁配置第一式與第二式；戰鬥時依順序循環施展，同一招不可重複裝配。'])+helpCard('淬劍與試劍境',['淬劍消耗劍元且不受練氣境界限制；每提升一層便開放下一關試劍境。','小層可直接提升；每逢第十層，需先擊敗對應試劍境關卡才能跨入下一大境界。','試劍境自動戰鬥期間會持續播放專屬樂曲，直到整次挑戰結束。','淬劍達凝魄並通過第40關後，可從破軍、流光、歸元三種劍意中擇一領悟。'])+helpCard('正邪閱歷與劍印',['門派任務會依門派立場累積正氣或邪氣，兩者不消耗也不互相抵銷。','淬劍大境界突破時，會依正邪閱歷凝成天罡、血煞或兩儀劍印；閱歷不足仍可突破，但不會凝印。','劍印會影響劍招效果與演出色彩。']),
    body:helpCard('鍛體方式',['煉體不消耗修為，也不受練氣境界限制；鍛體後累積淬鍊度，滿足條件即可提升。','基礎鍛體消耗較低且安全；藥浴鍛體可卸除負荷並舒緩傷勢；極限鍛體收益最高但受傷風險也最高。','體力每分鐘恢復1點，肉身負荷每分鐘消退1點；鍛體室需先正式開啟煉體路線。'])+helpCard('傷勢與肉身試煉',['擦傷降低淬鍊度收益；內傷提高鍛體體力消耗並降低試煉氣血；筋傷降低試煉閃避且禁止極限鍛體。','傷勢可等待自然痊癒，或消耗指定物資立即療傷。','小層在淬鍊度滿後可直接突破；每逢第十層需完成肉身試煉。','試煉失敗會保留已累積的淬鍊度，但可能留下傷勢。'])+helpCard('肉身特性',['玉骨降低極限鍛體受傷率；鳴髓縮短傷勢時間；曜身提高試煉氣血。','擎嶽降低試煉傷害；撼霄降低療傷消耗；鎮陸用於滿足凡間煉體路線的最終條件。']),
    battle:helpCard('通用戰鬥規則',['命骨影響氣血，元息影響攻擊，玄軀影響防禦，游影影響閃避，銳識影響命中與暴擊。','戰鬥會自動進行並顯示每回合招式、傷害與閃避結果。','戰鬥進行三回合後才可中途退出；除一般切磋外，中途退出視為認輸。','不同戰鬥會使用各自的場景、敵人數值、勝利條件與獎勵。'])+helpCard('戰鬥類型',['九鎖封天：依序挑戰主線關卡，取得靈石、素材及關卡獎勵。','試劍境：使用淬劍與本命劍相關能力擊敗劍道幻影。','肉身試煉：依煉體能力承受指定回合，重點是撐過考驗而非擊倒對手。','門人切磋：每日勝利次數有限；掌門挑戰需達指定職位與聲望。']),
    ...
    cave:helpCard('靈脈與修行設施',['洞府靈脈提供設施運作所需供應，提升靈脈可擴充供應上限。','聚靈室提高掛機修為；洗劍池提高掛機劍元；鍛體室協助累積淬鍊度。','設施可啟停與升級；供應不足時無法啟用，離線期間仍會依狀態結算。'])+helpCard('道童與資源生產',['道童可分配至食物、木材及隕鐵生產線；每條生產線每升一級可多安排一名道童。','三條生產線目前最高30級，因此單條生產線最多安排30名道童。','生產線等級影響產量，倉儲與相關設施影響可保留的離線資源。'])+helpCard('煉丹、煉器與儲物袋',['丹房使用副本主藥與丹砂製作永久屬性丹；可製作階級取決於三路最高境界。','器室使用副本素材、階材與器靈精魄製作裝備，裝備品質會影響數值範圍。','副本素材、丹藥、功法書與裝備會存入儲物袋；不需要的物品可在詳情中售出。','裝備比較中的綠色上箭頭代表提升，紅色下箭頭代表降低。']),
    sect:helpCard('加入、離開與重返',['一至九星門派會隨三路最高境界依序開放；無門無派時可隨機尋訪目前可加入的門派。','隨機尋訪會優先遇到未拜入或尚未探索完整的門派；聲望堂信物可指定加入對應門派。','脫離門派後，需等待三個修練年才能再次免費尋訪；使用指定門派信物不受此等待限制。','每個門派的職位、功勳、貢獻、任務、門人好感與傳承見聞都會獨立保存，日後重返可繼續累積。'])+helpCard('門派設施',['門派主殿：查看職位與資源、晉升、領取每日俸祿及脫離門派。','門人：聊天、送禮、切磋、向掌門請安或發起掌門挑戰。','練功房：進行每日練功及掌門傳功。','執事堂：承接持續任務；傳功殿：學習門派功法；功勳堂：使用貢獻兌換物資。','門派見聞：只記錄玩家親自拜入後得知的門派與傳承線索。'])+helpCard('功勳、貢獻與職位',['持續任務每個修練年同時增加本門功勳與門派貢獻，並發放靈石、聲望及對應正邪閱歷。','職位依累計本門功勳晉升，晉升不會扣除功勳；門派貢獻則保留給功勳堂兌換。','外門、內門、親傳、供奉、護法會影響功法、俸祿及部分門派功能。','曾在不同門派達到親傳會累積門派閱歷，使任務所得功勳與貢獻提高，最高加成50%。'])
...
function renderMarket(tab=currentMarketTab){
  ...
  const data={
    ...
    treasure:{title:'百寶樓',subtitle:'仙珍奇物',currency:'jade',products:['divineRoamingManual','mindEmbodimentManual']}
  }[tab];
...
load();normalizeSectRecords();delete state.mainlineEnemySnapshots;delete state.sectNpcSnapshot;normalizeDivineRoamingTiming();if(state.divineRoamingManualCount>0)state.divineRoamingUnlocked=true;normalizeMainlineMaterialItems();registerEquipmentItems();normalizeEquipmentLoadout();normalizeIndependentPaths();normalizeFirstPath();normalizeSwordPath();normalizeBodyPath();normalizeFirstPath();ensureSectTechniqueRevisionMail();
...
$('#spiritUp').onclick=()=>primaryPathAction(); $('#swordUp').onclick=()=>upgrade('sword'); $('#bodyUp').onclick=()=>openPrimaryBodyView('training');
