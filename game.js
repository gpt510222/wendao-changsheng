...
function primaryPathAction(){const path=state.activePath||state.firstPath||'spirit';if(path==='spirit')return upgrade('spirit');if(path==='sword'){if(!state.swordEmbryo)return openPrimarySwordView('sword');return upgrade('sword')}openPrimaryBodyView(bodyFoundationsReady()?'body':'training')}
...
function upgrade(type) {
  ...
  if((spirit&&state.spiritLevel>=maxSpiritLevel)||(sword&&(state.swordLevel||0)>=maxSwordLevel)||(!spirit&&!sword&&state.bodyLevel>=mortalBodyMaxLevel))return toast(!spirit&&!sword?'凡間肉身已至鎮陸十層極限':'已達此道最高境界');
...
function battlePlayerStats(){
  ...
  return {
    maxHp:Math.round(combatHealth(rootBone)*(activeBodyInjury()==='internal'?.85:1)),attack:Math.max(12,trueQi*5),qiAttack:Math.max(12,trueQi*5),bodyAttack:Math.max(12,(rootBone*2+physique*3)*(1+Math.min(.3,bodyRealmIndex()*.0375))),defense:Math.max(0,physique*20)*guardPower,
...
function renderHelp(tab='cultivation'){
  $$('.help-tabs button').forEach(button=>button.classList.toggle('active',button.dataset.helpTab===tab));const helpTabs=$('.help-tabs'),activeHelpTab=helpTabs?.querySelector('.active');if(helpTabs&&!helpTabs.dataset.horizontalScroll){helpTabs.dataset.horizontalScroll='true';helpTabs.addEventListener('wheel',event=>{if(Math.abs(event.deltaY)<=Math.abs(event.deltaX))return;helpTabs.scrollLeft+=event.deltaY;event.preventDefault()},{passive:false})}if(helpTabs&&activeHelpTab)helpTabs.scrollLeft=activeHelpTab.offsetLeft-helpTabs.offsetLeft-(helpTabs.clientWidth-activeHelpTab.clientWidth)/2;const pages={
    ...
    cave:helpCard('靈脈與修行設施',['洞府靈脈提供設施運作所需供應，提升靈脈可擴充供應上限。','聚靈室提高掛機修為；洗劍池提高掛機劍元；鍛體室降低手動鍛體的材料消耗與受傷風險。','設施可啟停與升級；供應不足時無法啟用。鍛體室不會在在線或離線期間自動增加肉身進度。'])+helpCard('道童與資源生產',['道童可分配至食物、木材及隕鐵生產線；每條生產線每升一級可多安排一名道童。','三條生產線目前最高30級，因此單條生產線最多安排30名道童。','生產線等級與倉儲容量是煉體大境界突破條件的一部分，跨入鎮陸需三線皆達30級。'])+helpCard('煉丹、煉器與儲物袋',['丹房使用副本主藥與丹砂製作永久屬性丹；可製作階級取決於三路最高境界。','器室使用副本素材、階材與器靈精魄製作裝備，裝備品質會影響數值範圍。','副本素材、丹藥、功法書與裝備會存入儲物袋；不需要的物品可在詳情中售出。','裝備比較中的綠色上箭頭代表提升，紅色下箭頭代表降低。']),
