...
function switchWorldRealm(){
  ...
  const entering=a.currentRealm!=='immortal',path=a.lastMortalTrainingGround||state.activePath||state.firstPath||'spirit',mortalScene=cultivationPathMeta[path]?.scene||cultivationPathMeta.spirit.scene,immortalScene='assets/qstyle-v2/ascension/immortal-realm-barren-v1.png',transition=document.createElement('div');
  realmSwitching=true;transition.className=`realm-transition realm-transition-${entering?'up':'down'}`;transition.innerHTML=realmTransitionScene(mortalScene,'凡間')+realmTransitionScene(immortalScene,'仙界');document.body.append(transition);
