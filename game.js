...
function processDivineRoaming(){divineRoamingDaily();let job=state.divineRoamingJob,changed=false,guard=0;const now=gameNow();while(job&&job.completed<job.total&&now>=job.nextAt&&guard++<100){const boundary=job.nextAt,stage=mortalMainline[job.stageId-1],attempt=(state.divineRoamingUsed||0)+1;rollDivineRoamingDrop(stage,divineRoamingRate(attempt));state.divineRoamingUsed=attempt;job.completed++;changed=true;if(job.pendingStageId){const target=job.pendingStageId,available=Math.max(0,100-state.divineRoamingUsed),total=Math.min(Math.max(0,Math.floor(job.pendingTotal||0)),available);if(total>0)state.divineRoamingJob={stageId:target,total,completed:0,startedAt:boundary,nextAt:boundary+divineRoamingAttemptMs};else{job.completed=job.total;job.finishedAt=boundary;delete job.pendingStageId;delete job.pendingTotal}}else if(job.completed>=job.total)job.finishedAt=boundary;else job.nextAt=boundary+divineRoamingAttemptMs;job=state.divineRoamingJob}if(changed)save();return changed}
function divineRoamingStatus(){processDivineRoaming();const job=state.divineRoamingJob;if(!job)return '神念歸體・目前閒置';const stage=mortalMainline[job.stageId-1];if(job.completed>=job.total)return `${stage.name}遠遊完成・${job.completed}/${job.total}`;const pending=job.pendingStageId?`・本次結束後轉往${mortalMainline[job.pendingStageId-1].name}`:'';return `正遠遊${stage.name}・${job.completed}/${job.total}${pending}`}
...
function joinSect(pick){
  ...
  state.sect=pick.name;state.sectFaction=pick.faction;state.sectStar=pick.star;state.sectContribution=0;state.sectRank=0;state.sectTask='';state.sectJoinedAt=gameNow();state.sectYearsProcessed=0;state.actingLeader=false;state.npcAffinity={};state.npcDaily={};
...
function renderHelp(tab='cultivation'){
  $$('.help-tabs button').forEach(button=>button.classList.toggle('active',button.dataset.helpTab===tab));const pages={
    cultivation:helpCard('三路修行',['修為只用於練氣；淬劍使用獨立累積的劍元；煉體則以體力鍛鍊並累積淬鍊度。','修為與劍元採大數精確保存，可長期囤積；數量極大時會自動改用更高單位或科學記號。','新手教程只正式開啟練氣；消耗30隕鐵可開啟淬劍，消耗120食物可開啟煉體。','主畫面的淬劍按鈕會直接消耗劍元提升境界；只有每逢第十層才會引導至試劍境。煉體按鈕則進入煉體頁面。'])+helpCard('境界、功能與關卡解鎖',['練氣、淬劍、煉體皆為每境十層；主畫面會直接顯示下一層所需資源，資源足夠後方可提升。','九鎖封天每一境有兩關：需先通過前一關，並讓練氣、淬劍或煉體任一路線達到該境，才會開啟下一關。','淬劍每提升一層開放一關試劍境；逢第十層必須先通過同層關卡。煉體逢第十層則必須完成對應肉身試煉。','意念入體可使用百寶樓的意念入體訣提前習得，未提前習得者會在化念一層自動領悟；化念一層也可取得神念遠遊訣，開通已通關副本的神念遠遊。','坊市、藏經閣與聲望堂樓層依任一路線的最高進度開放；上樓時會顯示所需境界。','丹房與器室可查看一至九階配方，但實際可製作階級取決於任一路線目前抵達的境界。'])+helpCard('練氣渡劫',['渡劫有成功率，可投入對應境界的渡劫丹，每顆提升 5%。','成功後提升境界與屬性；失敗會損失本次所需修為的一半。','渡劫演出期間會鎖定其他操作，成功或失敗後需由玩家手動退出。'])+realmHelp(),
    ...
    arts:helpCard('靈根',['靈氣由掛機結算獲得，可用於提升金、木、水、火、土五行靈根。','靈根會放大對應五行功法的效果；天契會提高靈氣獲取效率。','靈池可提升並容納更多靈氣，操作前請留意畫面顯示的消耗。'])+helpCard('功法',['功法分為門派功法、功法書與招式三大頁；功法書內再分玄錄、命篇、體典、行章、悟卷與天箋。','門派功法向大長老學習，受門派職位、技能格上限與貢獻影響。','功法書可從藏經閣購買並於儲物袋使用；同名功法僅能學習一次。','意念入體可由意念入體訣提前習得，或於化念一層自動領悟，用於開啟人物屬性且不列入功法頁籤。','功法加成會合併進入人物屬性與戰鬥力計算。']),
