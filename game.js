...
function learnedSectSkills(){return (state.learnedArts||[]).filter(art=>art.kind==='sectSkill'&&!art.legacySectTechnique)}
function combatTechniqueById(id){return startingTechniques.find(move=>move.id===id)||swordTechniqueCatalog.find(move=>move.id===id)||learnedSectSkills().find(move=>move.id===id)}
function combatTechniqueAvailable(move){return !!move&&(move.kind==='sectSkill'?learnedSectSkills().some(art=>art.id===move.id):move.id==='origin'?!!state.spiritPathOpened:move.id==='body-origin'?!!state.bodyPathOpened:swordTechniqueUnlocked(move.id))}
...
function renderArtsPanel(view='sect',preserveScroll=false){
  ...
  const selectedBookKind=artBookTabs.some(([key])=>key===view)?artKinds[view]:null,selectedBookTotal=selectedBookKind?learned.filter(art=>art.kind===view).reduce((sum,art)=>sum+artTotalEffect(art),0):0;
  const wallet=view==='moves'?'':`<div class="arts-wallet"><span id="artsAuraAmount">當前靈氣 ${formatLargeNumber(state.aura)}</span>${selectedBookKind?`<span class="arts-kind-total">${selectedBookKind.tab}累積${selectedBookKind.label} ${formatLargeNumber(selectedBookTotal)}</span>`:''}${view==='sect'?`<span>門派技能 ${sectLearned.length} / ${cap}</span>${cap<40?`<button id="expandArtsBtn" ${state.spiritStone>=expand?'':'disabled'}>擴充一格・${formatLargeNumber(expand)}靈石</button>`:'<b>已達40格上限</b>'}`:''}</div>`;
...
function animateBattleStrike(attacker,target,damage,technique){
  ...
  if(technique.kind==='sectSkill')fx.dataset.skillName=technique.name;
