...
function renderBagView(view) {
  ...
  if(view==='bag') {
    const items=syncBagItemOrder(Object.entries(itemCatalog).filter(([,item])=>(state[item.count]||0)>0)),capacity=bagCapacity(),used=items.length,cost=bagUpgradeCost();
    ...
    inner.innerHTML=`<section class="bag-toolbar"><div><small>儲物袋品階</small><b>${bagRankNames[state.bagRank-1]}階</b><span>${used} / ${capacity} 格</span></div><div class="bag-toolbar-actions"><button id="openBagUpgradeBtn" ${state.bagRank>=16?'disabled':''}>${state.bagRank>=16?'已滿階':bagUpgradeDetailsOpen?'收起':'升階'}</button><button id="organizeBagBtn">整理</button></div></section>${bagUpgradeDetailsOpen&&state.bagRank<16?`<section class="bag-upgrade-details"><img src="assets/qstyle-v2/mending-silk-cutout.png" alt="補天絲"><span><small>補天絲</small><b>${formatLargeNumber(state.mendingSilk||0)} / ${formatLargeNumber(cost)}</b><em>升至${bagRankNames[state.bagRank]}階・容量增加 10 格</em></span><button id="confirmUpgradeBagBtn" ${(state.mendingSilk||0)>=cost?'':'disabled'}>確認升階</button></section>`:''}<div class="inventory-grid">${itemButtons}${emptySlots}</div><small class="empty-note">${used?'點擊道具可查看詳細資訊':'目前儲物袋空空如也'}</small>`;
