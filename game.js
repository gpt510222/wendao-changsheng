...
async function startGame() {
  if(sessionOnline)return;finishPause();let serverSettlement;try{serverSettlement=await connectServerPlayerState()}catch(error){toast(`需要連線伺服器才能進入遊戲：${error.message}`);return}sessionOnline=true;await importLegacyQiRuntime();await playerQiRuntime();await playerSectBootstrap();await playerSectProgressBootstrap();await playerSectEconomy();await syncServerJade();
