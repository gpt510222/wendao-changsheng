...
function ensureAscensionModal(){let modal=$('#ascensionModal');if(modal)return modal;modal=document.createElement('div');modal.id='ascensionModal';modal.className='ascension-modal';modal.innerHTML='<section class="ascension-window"><button class="ascension-close" type="button" aria-label="離開天路">×</button><div id="ascensionContent"></div></section>';document.body.append(modal);modal.querySelector('.ascension-close').onclick=()=>{const content=$('#ascensionContent'),stage=content?.querySelector('.ascension-dialogue-stage'),chapter=content?.querySelector('.ascension-chapter-card small')?.textContent||'',locked=!!content?.querySelector('.ascension-allocation')||!!stage&&!chapter.includes('序章');if(locked)return toast('此飛升關卡進行中，無法退出');modal.classList.remove('show');resumeWorldBgm()};return modal}
