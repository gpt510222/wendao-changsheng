...
function normalizeAscension(){const base=defaults.ascension,current=state.ascension&&typeof state.ascension==='object'?state.ascension:{};state.ascension={...base,...current,heartAnswers:Array.isArray(current.heartAnswers)?current.heartAnswers.slice(0,3):[],delusionReplies:Array.isArray(current.delusionReplies)?current.delusionReplies.slice(0,3):[]};const a=state.ascension;if(['qi','sword','body'].includes(a.route))a.prologueCompleted=true;if(a.heartRewardAllocated)a.heartTrialCompleted=true;if(a.delusionRewardAllocated){a.heartTrialCompleted=true;a.heartRewardAllocated=true;a.delusionTrialCompleted=true}if(a.roadEndRewardAllocated||a.ascended){a.heartTrialCompleted=true;a.heartRewardAllocated=true;a.delusionTrialCompleted=true;a.delusionRewardAllocated=true;a.roadEndCompleted=true;a.roadEndRewardAllocated=true}return a}
