...
function normalizeBodyPath(){const stamina=Number(state.bodyStamina),now=gameNow(),version=Math.floor(state.bodyTrainingSystemVersion||0);if(version<4&&state.bodyPathOpened){const old=Math.max(0,Math.floor(state.bodyLevel||0));state.bodyLevel=old>=80?Math.min(71,68+Math.floor((old-80)*4/10)):Math.min(67,Math.floor(old*68/80));state.bodyFoundations={bone:0,blood:0,organs:0}}state.bodyLevel=Math.max(0,Math.min(maxBodyLevel,Math.floor(state.bodyLevel||0)));state.bodyStamina=Number.isFinite(stamina)?Math.max(0,stamina):100;state.bodyStaminaUpdatedAt=Number(state.bodyStaminaUpdatedAt)||now;state.bodyTemper=Math.max(0,Math.floor(state.bodyTemper||0));state.bodyTrainingLoad=Math.max(0,Math.min(100,Number(state.bodyTrainingLoad)||0));state.bodyTrainingLoadUpdatedAt=Number(state.bodyTrainingLoadUpdatedAt)||now;if(!bodyInjuries[state.bodyInjury]){state.bodyInjury='';state.bodyInjuryUntil=0}state.bodyFoundations=state.bodyFoundations&&typeof state.bodyFoundations==='object'?{bone:Math.max(0,Number(state.bodyFoundations.bone)||0),blood:Math.max(0,Number(state.bodyFoundations.blood)||0),organs:Math.max(0,Number(state.bodyFoundations.organs)||0)}:{bone:0,blood:0,organs:0};state.bodyTrainingCharges=Math.max(0,Math.min(14,Math.floor(Number(state.bodyTrainingCharges)||2)));state.bodyTrainingChargeUpdatedAt=Number(state.bodyTrainingChargeUpdatedAt)||now;state.bodyTrialFailures=state.bodyTrialFailures&&typeof state.bodyTrialFailures==='object'?state.bodyTrialFailures:{};state.bodyBreakthroughValue=Math.max(0,Math.floor(state.bodyBreakthroughValue||0));state.bodyPassiveUpdatedAt=Number(state.bodyPassiveUpdatedAt)||now;state.bodyTrainingSystemVersion=4;refreshBodyState();refreshBodyTrainingCharges();syncBodyTemperFromFoundations();if(state.bodyLevel>=maxBodyLevel)unlockTitle('body-ultimate')}
...
function bodySessionNeed(level=state.bodyLevel){const realm=Math.floor(Math.max(0,level)/bodyStagesPerRealm);if(realm<6)return 4;if(realm<12)return 5;if(realm<17)return 6;return [20,70,110,170,240,310][Math.min(5,realm-17)]}
...
async function fetchImmortalLeaderboard(){const query='select=user_id,player_name,game_version,updated_at&limit=1000',response=await fetch(`${leaderboardConfig.url}/rest/v1/player_rankings?${query}`,{headers:leaderboardHeaders()});if(!response.ok)throw new Error('immortal ranking fetch failed');return (await response.json()).map(parseImmortalLeaderboardRecord).filter(Boolean).sort((a,b)=>b.outpostStage-a.outpostStage||b.wastelandProgress-a.wastelandProgress||b.bodyStage-a.bodyStage||b.restorationStage-a.restorationStage||b.exploration-a.exploration||b.expeditionRuns-a.expeditionRuns||String(a.updated_at).localeCompare(String(b.updated_at))).slice(0,leaderboardConfig.limit)}
...
function finishBattle(won,reason){
  ...
  else if(battle.mode==='bodyTrial'){const trialLevel=battle.bodyTrialLevel||state.bodyLevel+1,key=String(trialLevel);if(won){if(trialLevel===state.bodyLevel+1&&bodyFoundationsReady()&&bodyBreakthroughMaterialsReady(battle.bodyRequirement)){consumeBodyBreakthroughMaterials(battle.bodyRequirement);state.bodyLevel++;resetBodyFoundations();state.bodyTrialFailures[key]=0;applyAttributeGain(bodyAttributeGain(state.bodyLevel));if(state.bodyLevel>=maxBodyLevel)unlockTitle('body-ultimate');reward=` 突破物資已投入，肉身突破至${bodyRealmName(state.bodyLevel)}。`}else reward=' 試煉資格已失效，未完成突破。'}else{state.bodyTrialFailures[key]=(state.bodyTrialFailures[key]||0)+1;const injury=Math.random()<.6?'internal':'tendon';inflictBodyInjury(injury);reward=` 突破物資完整保留；留下${bodyInjuries[injury].name}。此境已失敗 ${state.bodyTrialFailures[key]} 次。`}}
...
function renderAscensionEntrances(){const a=normalizeAscension(),eligible=ascensionEligible(),realmButton=$('#realmSwitchButton'),realmImage=realmButton?.querySelector('img');$('#ascensionButton').classList.toggle('hidden',!eligible);realmButton.classList.toggle('hidden',!a.ascended);const immortal=a.currentRealm==='immortal'&&a.ascended,$scene=$('.scene-bg'),inWasteland=a.immortalRealm?.region==='wuxi-wasteland';$('#gameScreen').classList.toggle('immortal-realm',immortal);$('#gameScreen').classList.toggle('wuxi-wasteland',immortal&&inWasteland);$$('.path-action-group').forEach(group=>group.classList.toggle('realm-hidden',immortal));if(immortal){$scene.src=inWasteland?'assets/qstyle-v2/ascension/immortal-wuxi-wasteland-v1.png':'assets/qstyle-v2/ascension/immortal-realm-barren-v1.png';$scene.alt=inWasteland?'寂靜荒蕪的無息荒原':'靈氣枯竭的荒蕪仙界';realmImage.src='assets/qstyle-v2/ascension/entry-mortal-v1.png';realmImage.alt='凡間';realmButton.setAttribute('aria-label','返回凡間');$('#headerSpiritRealm').title='仙界沒有靈氣；凡間修練與產能照常運作'}else{const path=a.lastMortalTrainingGround||state.activePath||state.firstPath||'spirit';$scene.src=cultivationPathMeta[path]?.scene||cultivationPathMeta.spirit.scene;$scene.alt=`${cultivationPathMeta[path]?.name||'練氣'}修練道場`;realmImage.src='assets/qstyle-v2/ascension/entry-immortal-v1.png';realmImage.alt='仙界';realmButton.setAttribute('aria-label','前往仙界')}}
