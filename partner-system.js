...
function partnerRenderEncounter(event,content){
 ...
 if(step?.name){content.innerHTML=`<section class="partner-story" style="--partner-scene:url('${partnerScene(event.chapter)}')"><div class="partner-visual-stage"><img class="partner-dialogue-art" src="${partnerArt('dialogue')}" alt="${story.gender}修士半身像"></div><div class="partner-dialogue-card">${intro}<small>${event.title}</small><h2>${speaker}</h2><p>${step.text}</p></div><div class="partner-choice-panel partner-name-panel"><label class="partner-name-field">為這位${story.gender}修士題名<input id="partnerNameInput" maxlength="8" placeholder="輸入姓名"></label><button id="partnerNameConfirm" class="jade-button">記下姓名</button></div></section>`;document.querySelector('#partnerNameConfirm').onclick=()=>partnerConfirmName(event);return}
 const choices=(step?.choices||[]).map(args=>partnerChoice(...args));event.choices=choices;
 content.innerHTML=`<section class="partner-story" style="--partner-scene:url('${partnerScene(event.chapter)}')"><div class="partner-visual-stage"><img class="partner-dialogue-art" src="${partnerArt('dialogue')}" alt="${story.gender}修士半身像"></div><div class="partner-dialogue-card">${intro}<small>${event.title}・${partnerRoutes[story.route]}・${partnerPersonalities[story.personality]}</small><h2>${speaker}</h2><p>${step.text}</p></div><div class="partner-choice-panel encounter-choices">${choices.map((c,i)=>`<button data-encounter-choice="${i}"><b>${c.label}</b></button>`).join('')}</div></section>`;document.querySelectorAll('[data-encounter-choice]').forEach(b=>b.onclick=()=>partnerResolveChoice(event,+b.dataset.encounterChoice))
}
...
function partnerRenderEnding(event,content){const s=state.partnerStory,eligible=partnerEndingEligible(),ending=eligible?'兩心同道':s.bond>=3?'有情未同道':s.accord>=3?'同道未有情':'各自長生';const question=eligible?`「${state.name}，往後的長生路，你可願與我同行？」`:`你們在夕照中說完最後一段話。此心或近、此道或同，卻終究沒有走成同一條路。`;event.choices=eligible?[partnerChoice('我願意'),partnerChoice('仍選擇獨行')]:[partnerChoice('記住這段歲月')];content.innerHTML=`<section class="partner-story partner-ending" style="--partner-scene:url('${partnerScene(10)}')"><div class="partner-visual-stage"><img class="partner-dialogue-art" src="${partnerArt('dialogue')}" alt="${s.gender}修士半身像"></div><div class="partner-dialogue-card"><small>${event.title}・${ending}</small><h2>${partnerDisplayName()}</h2><p>${question}</p></div><div class="partner-choice-panel encounter-choices">${event.choices.map((c,i)=>`<button data-partner-ending="${i}"><b>${c.label}</b></button>`).join('')}</div></section>`;document.querySelectorAll('[data-partner-ending]').forEach(b=>b.onclick=()=>partnerResolveEnding(event,+b.dataset.partnerEnding))}
